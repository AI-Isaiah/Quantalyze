...
def test_python_writer_census_counts() -> None:
    ...
    Census (re-measured 2026-08-25 against the post-161.1-CR-02 tree):
      ⬆️ **+1 on 2026-08-25 (Phase 161.1 / F3).** ``analytics_runner`` went from 7
        ...
        ⬆️ **+1 on 2026-09-06 (Phase 164.2 plan 08).** ``EXPECTED_KEPT_VIA_N1``
        went 7 → 8 and NOTHING moved kept→skipped: ``_upsert_restore``'s payload
        is now BOUND to a local (``_restore_payload``) instead of written inline
        in the ``upsert()`` call, so it resolves through the n1 ``ast.Name`` arm
        like the other seven rather than through the direct-literal arm. The
        binding is required, not cosmetic — ``services/strategy_analytics_
        provenance.upsert_or_drop_provenance`` must be able to POP the two
        ``computation_error_*`` marker keys out of that exact dict and re-issue
        it if the database refuses them (TODOS ``[PROV-WRITER-23514]``), and an
        inline literal is not a thing the retry can reach. The site is still one
        readable literal at one call site, which is all this census asks of it.
