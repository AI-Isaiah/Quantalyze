...
def test_h11_sfox_admitted_to_onboard_and_resync_only(monkeypatch):
    ...
    # REJECTED: sfox on the fill-based / synchronous flows.
    for flow in ("teaser", "internal_report", "csv"):
        with pytest.raises(ValidationError, match="H-11"):
            Body(
                flow_type=flow,
...
def test_process_key_sfox_onboard_draft_carries_trust_tier_api_verified(client, monkeypatch):
    ...
    # The strategy_verifications draft insert carried trust_tier='api_verified'.
    insert_payloads = [
        c.args[0] for c in fake.table.return_value.insert.call_args_list if c.args
    ]
    drafts = [p for p in insert_payloads if p.get("source") == "sfox"]
    assert drafts, "no strategy_verifications draft insert captured for sfox"
    assert drafts[0]["trust_tier"] == "api_verified"


def test_resolve_asset_class_sfox_is_crypto_365():
    """SFOX-05 (MD-01): sfox resolves to asset_class='crypto' (√365 RISK basis,
    the #597 convention) WITHOUT touching supabase — closing the unknown
    -asset_class √252 blend-underestimation class for a sole sfox crypto leg."""
    from routers.process_key import _resolve_asset_class
    from services.closed_sets import CRYPTO_VENUES

    assert "sfox" in CRYPTO_VENUES
    fake = MagicMock()
    assert _resolve_asset_class("sfox", "s1", fake) == "crypto"
