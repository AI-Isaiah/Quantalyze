...
async def run_csv_strategy_analytics(strategy_id: str) -> dict:
    """CSV → analytics pipeline Task 5. Analytics pipeline for source='csv'
    strategies. Loads csv_daily_returns, builds pd.Series, calls
    compute_all_metrics. No position reconstruction, fills, trade_mix,
    or volume metrics (CSV has none of those — those panels stay null).

    Mirrors the structure of run_strategy_analytics but skips the exchange-
    specific branches. Sets computation_status='computing' on entry;
    'complete' or 'failed' on exit.
    """
    supabase = get_supabase()
    ...
    try:
        ...
        import pandas as pd  # already available at module level via benchmark dep; local import avoids top-level cycle risk
