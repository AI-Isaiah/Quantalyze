...
# ---------------------------------------------------------------------------
# Phase 106 / D7 (BB-03) — recurring `computing`-row janitor
# ---------------------------------------------------------------------------
#
# Promotes the one-time deploy script `scripts/reset_stuck_computing_rows.py`
# into a recurring tick (Vercel Cron -> Next proxy -> this endpoint, */15).
# It reaps `strategy_analytics` rows stranded at computation_status='computing'
# past a stale threshold, so no strategy stays stuck in `computing` forever
# when a worker/pod dies between the INSERT and the terminal UPDATE.

# Stale threshold for reaping a `computing` row. Derived from the watchdog
# ceiling, NOT a fixed guess: the max entry in
# `main_worker.py::WATCHDOG_PER_KIND_OVERRIDES` is 40 min (`process_key_long`,
# a 90-day archive backfill). The janitor must never race a legitimately-slow
# in-flight job, so the threshold = max ceiling (40) + headroom. Re-grep the
# map if a longer-running kind is added; keep threshold >= max ceiling + 15.
STALE_COMPUTING_THRESHOLD_MINUTES = 60

# Non-terminal compute_jobs statuses. A row in ANY of these for the same
# strategy_id COLUMN means a worker is actively handling it (e.g. a live
# process_key_long onboarding — coherence CHECK
# 20260710130000_stitch_composite_kind.sql:93 forces those rows to carry a
# NON-NULL strategy_id column), so the candidate is skipped, never reaped.
# One place to look for the grep-gate and future kinds.
...
# Error message stamped on a reaped row — names the janitor so the operator
# (and the user retrying) can tell a reap apart from a genuine compute failure.
JANITOR_REAP_ERROR_MESSAGE = (
    "Computation stalled and was reaped by the stuck-computing janitor. "
    "Please retry."
)


@router.post("/cron-janitor")
async def cron_janitor() -> dict[str, Any]:
    """Recurring cron endpoint: reap `strategy_analytics` rows stranded at
    computation_status='computing' past STALE_COMPUTING_THRESHOLD_MINUTES.

    Three-step sweep (seeded from scripts/reset_stuck_computing_rows.py):
      1. SELECT `computing` rows whose `updated_at` is older than the stale
         threshold (a Python-computed ISO timestamp — PostgREST `.lt()` sends
         the value as a string literal, so a raw SQL interval would be compared
         lexicographically instead of evaluated).
      2. For each, probe `compute_jobs` for a non-terminal row matched on the
         `strategy_id` COLUMN. A live job -> skip (the worker owns it).
      3. Otherwise compare-and-set the row to 'failed', filtering on
         `computation_status='computing'` so a worker that flips the row
         concurrently wins (idempotent, no-stomp).

    Auth: the X-Service-Key middleware (main.py) covers all /api/* routes, so
    no in-route auth is needed. Fail-loud: an unexpected top-level exception
    raises HTTPException(500) — the cron runner only alarms on non-2xx (a 200
    body with an `error:` field is silently treated as success). Per-row
    failures are logged and do NOT abort the batch.

    Returns {scanned, reaped, skipped_active}.
    """
    try:
        supabase = get_supabase()
        ...
        threshold = (
            ...
            - timedelta(minutes=STALE_COMPUTING_THRESHOLD_MINUTES)
        ).isoformat()
        ...
        if not candidates:
            logger.info("cron_janitor: no stranded computing rows found")
            return {"scanned": 0, "reaped": 0, "skipped_active": 0}

        logger.info(
            "cron_janitor: %d computing row(s) older than %d min",
            len(candidates),
            STALE_COMPUTING_THRESHOLD_MINUTES,
        )
        ...
        for row in candidates:
            sid = row["strategy_id"]
            try:
                # Column-probe active-job check — a live onboarding always
                # carries the strategy_id column (coherence CHECK above), so
                # this catches it and skips the reap.
                jobs = (
                    supabase.table("compute_jobs")
                    .select("id")
                    .eq("strategy_id", sid)
                    .in_("status", NON_TERMINAL_JOB_STATUSES)
                    .limit(1)
                    .execute()
                )
                if jobs.data:
                    skipped_active += 1
                    logger.info(
                        "cron_janitor: SKIP %s — active compute_jobs row exists",
                        sid,
                    )
                    continue

                # Compare-and-set: only flip a row still at 'computing'. A
                # worker that finished it between the SELECT and now wins.
                supabase.table("strategy_analytics").update(
                    {
                        "computation_status": "failed",
                        "computation_error": JANITOR_REAP_ERROR_MESSAGE,
                    }
                ).eq("strategy_id", sid).eq(
                    "computation_status", "computing"
                ).execute()
                reaped += 1
                logger.info("cron_janitor: REAP %s -> failed", sid)
            except Exception:
                # Per-row isolation: one bad row (transient DB blip, deadlock)
                # must not abort the sweep. logger.exception so the traceback
                # reaches Sentry; the next tick retries the row.
                logger.exception(
                    "cron_janitor: per-row reap failed for strategy %s — "
                    "continuing batch",
                    sid,
                )

        logger.info(
            "cron_janitor complete: scanned=%d reaped=%d skipped_active=%d",
            len(candidates),
            reaped,
            skipped_active,
        )
        ...
        # Fail-loud: a top-level failure (e.g. the initial SELECT blew up)
        # raises 500 so the cron runner alarms rather than silently treating a
        # 200 body as a healthy tick.
        logger.exception(
            "cron_janitor: unexpected top-level failure (%s)", type(exc).__name__
        )
        raise HTTPException(
            status_code=500,
            detail=f"cron_janitor failed: {type(exc).__name__}",
        ) from exc
