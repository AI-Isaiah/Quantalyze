...
// `equityCurveToDailyReturns` / `resolveDailyReturnSeries` moved to
// `@/lib/portfolio-math-utils` — the canonical home of `DailyPoint` and
// `normalizeDailyReturns` — so a caller that needs only the column-drift
// resolver doesn't pull `build-payload` (and the bundled BTC/SPX/ETH/GLD/IEF
// benchmark series) into its graph. Re-exported here so the factsheet /
// discovery-detail importers keep their existing paths.
export { equityCurveToDailyReturns, resolveDailyReturnSeries };
