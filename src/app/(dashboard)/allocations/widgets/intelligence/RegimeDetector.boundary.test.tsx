...
describe("RegimeDetector — withWidgetBoundary wiring", () => {
  it("renders the shared error card when data fails the schema", () => {
    render(<RegimeDetector data={{ strategies: "x" }} {...base} />);
    expect(screen.getByRole("alert")).toBeTruthy();
  });

  it("renders (insufficient-data state, not an error) for a valid empty payload", () => {
    render(<RegimeDetector data={{ strategies: [], analytics: null }} {...base} />);
    expect(screen.queryByRole("alert")).toBeNull();
    expect(screen.getByText(/Insufficient data for regime detection/i)).toBeTruthy();
  });
});
