...
/**
 * Portfolio documents (`/portfolios/[id]/documents`) error boundary. Catches
 * errors thrown by the RSC page (portfolio / documents fetch failure, RLS
 * denial). Surfaces the `digest` ONLY — never the thrown message (ASVS V7 /
 * Information Disclosure, T-53-13).
 */
export default function PortfolioDocumentsError({
  error,
  unstable_retry,
}: {
  ...
  useEffect(() => {
    console.error("[portfolio-documents-error]", error);
  }, [error]);
