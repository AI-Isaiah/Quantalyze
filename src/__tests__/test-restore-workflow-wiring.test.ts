...
describe("no lookup index reaches a narrowing call unchecked, anywhere in src/__tests__", () => {
   ...
   * file+function tolerance, a brace-matching span helper, and a calibration for
   * both. MEASURED 2026-09-10: that calibration could not fail (its "the
   * tolerance does not leak" legs called the UNTOLERATED scan path, so deleting
   * the tolerance left them green), and the machinery outweighed the one 80-line
   * file it policed. Removed, tolerance and all. The helper is still the named
   * home for deliberate demonstrations — it is simply not part of the surface.
  ...
  /**
   * Where a DELIBERATE degeneracy demonstration belongs (Phase 164.8.2 / W3).
   * Several calibrations in scanned files must reproduce the offending
   * expression to measure the trap it replaces; they call this helper rather
   * than writing it inline, so the rule has nothing to tolerate. The helper is
   * NOT scanned — see the SCAN_FILES docblock above.
   */
