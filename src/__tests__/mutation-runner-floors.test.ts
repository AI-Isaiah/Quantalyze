...
describe("164.3.1-10 — CI re-asserts the cross-check out of process (the anti-parse-only pattern, extended)", () => {
  ...
  const GREEN_LOG = [
    ...
    "lane-invocations: 189   (arm lanes actually spawned — tallied inside runLane, independent of the 189 the verdict loop counted; plus 17 baseline / 17 restore leg(s))",
    // 164.4-01: the per-file breakdown. These SEVENTEEN rows are the real,
    // measured shape at plan 164.4-07, which annotated the csv-finalize-fold /
    // funding-fees / allocator-derived-equity / user-notes batch: their
    // `biting` column must SUM to the aggregate `biting:` above (6 + 7 + 10 + 7
    // + 9 + 7 + 7 + 7 + 15 + 15 + 11 + 9 + 8 + 9 + 45 + 6 + 11 = 189) and their
    // COUNT must equal the `coverage:` numerator (17), both of which the
  ...
  it("GREEN: a log whose two tallies agree passes and says so", () => {
    ...
    expect(r.out).toContain("189 arm lane(s) spawned");
  });
  ...
  it("RED: more per-file rows than the coverage numerator claims fails, naming both counts", () => {
    const extra = GREEN_LOG.replace(
      ...
      (line) =>
        `${line}\n  file test_an_eighteenth_annotated_gate.sql: sections 15 / judged 0 / annotated 0 / waived 0 / biting 0`,
    );
    expect(extra).not.toBe(GREEN_LOG);
    const r = runCountRecheck(extra);
    expect(r.status, r.out).toBe(1);
    expect(r.out).toContain("printed 18 per-file row(s) but reported 17 annotated file(s)");
    expect(r.out).not.toContain("two tallies agree");
  });
  ...
  it("RED: a per-file biting column that does not SUM to the aggregate fails, naming both sums", () => {
    ...
    expect(r.status, r.out).toBe(1);
    expect(r.out).toContain("rows sum to 188 biting arm(s) but the aggregate");
    expect(r.out).toContain("reports 189");
    expect(r.out).not.toContain("two tallies agree");
  });
  ...
  it("RED: the parse-only shape — 189 executed claimed, 0 lanes counted — fails naming all three numbers", () => {
    const severed = GREEN_LOG.replace(/^lane-invocations: 189 /m, "lane-invocations: 0 ");
    expect(severed).not.toBe(GREEN_LOG);
    const r = runCountRecheck(severed);
    expect(r.status, r.out).toBe(1);
    expect(r.out).toContain("GATE failing, not the corpus");
    expect(r.out).toContain("executed=189 lane-invocations=0 biting=189");
    expect(r.out).not.toContain("two tallies agree");
  });

  it("RED: a single unaccounted lane also fails — the relation is exact", () => {
    const extra = GREEN_LOG.replace(/^lane-invocations: 189 /m, "lane-invocations: 190 ");
    const r = runCountRecheck(extra);
    expect(r.status, r.out).toBe(1);
    expect(r.out).toContain("executed=189 lane-invocations=190 biting=189");
  });

  it("RED: a NON-NUMERIC lane-invocations count is a MEASURE_FAIL, never parsed as a number", () => {
    const garbled = GREEN_LOG.replace(/^lane-invocations: 189 /m, "lane-invocations: abc ");
  ...
  it("RED: a waived count above WAIVED_CEILING fails naming both numbers — the W field is read, not ignored", () => {
    // The GREEN log carries `arms: 189/189/0`; one waiver against a ceiling read
    ...
    const waived = GREEN_LOG.replace(/^arms: 189\/189\/0 /m, `arms: 189/189/${WAIVED_CEILING + 1} `);
  ...
  it("the siblings still bite in the same extracted block — executed-is-zero and biting-above-executed", () => {
    ...
    const zero = GREEN_LOG.replace(/^arms: 189\/189\/0 /m, "arms: 0/189/0 ");
    const z = runCountRecheck(zero);
    expect(z.status, z.out).toBe(1);
    expect(z.out).toContain("ZERO arms executed");
    const spliced = GREEN_LOG.replace(/^biting: 189 /m, "biting: 190 ");
    const s = runCountRecheck(spliced);
    expect(s.status, s.out).toBe(1);
    expect(s.out).toContain("biting (190) exceeds executed (189)");
  });
});
