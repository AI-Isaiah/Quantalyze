...
const CONTRACT_GUARDS: Guard[] = [
  ...
  { path: "src/lib/wizard/derive-resume-overrides-arity.contract.test.ts", batch: "164.2.1 / SESSIONID-FENCE", invariant: "every PRODUCTION call of `deriveWizardResumeOverrides` passes all FOUR arguments. Its 4th parameter defaults to `null` and `null` is the PERMISSIVE value, so an omitted argument silently DISABLES the key fence with no type error and no other failing test — a fail-open default. Making the parameter required is refused by success criterion 5 (the ~25 existing three-argument unit pins must pass UNMODIFIED, which is what proves the default did not change behaviour for a caller making no key claim), so the guard closes the production side instead. ROW 1: population WALKED FROM DISK every run; test files are the deliberate exemption (they exercise the three-argument contract). COMMENT-STRIPPED via `stripCommentsPreserveLines` — six comments in two files mention the symbol beside its call site, so a raw grep would score paragraphs as call sites (DEF-16-2). Both-polarity self-tests cover arity 0/3/4/5, a trailing comma, commas nested in calls/objects/arrays/strings, the declaration, and the comment specimen; a non-empty-population assertion fences the rename/broken-walk vacuity. Registered by 164.2.1 wave 2" },
  ...
  { path: "src/__tests__/contracts/analytics-deploy-tree-compare.contract.test.ts", batch: "DEPLOYVERIFY-SHA-NOT-CODE", invariant: "analytics-deploy-verify.yml decides convergence on the `analytics-service` TREE OBJECT, not on the commit SHA — pinned by EXTRACTING the probe step's own shell out of the workflow, stubbing `curl`/`git`/`sleep` on PATH and EXECUTING it, never by grepping the YAML. WHY: the probe compared SHAs while its question is about CODE, so a merge changing no service code left prod on an older SHA forever and filed P1 staleness about a service running the right bytes. MEASURED 2026-09-13 against the Railway API: merge f10b0e23 (two .planning/ files) left prod on 0b9f0699 with the analytics-service tree byte-identical (e6d8e33f). \u26d4 Railway did NOT decline the deploy — it QUEUED one (b076139c) and left it WAITING on the check-suite, which contains this workflow's own `verify` check; that check loops while prod has not converged, and prod cannot converge while Railway holds the deploy behind the suite. A CIRCULAR WAIT, distinct from the 2026-06-21 red-check-skips incident: this is an INCOMPLETE check making Railway WAIT, costing a ~80-minute deploy delay on every docs-only merge. Exiting in seconds is what releases it; issue #751 had been open since 2026-09-07 with nine comments and its ORIGINAL pair (05994f1d vs 45218594) carries the same identical-tree signature (b2f9f92e), so every comment was triaged against a cause never present. BOTH POLARITIES: S2 proves a docs-only merge converges, and S3 (differing tree) plus S4 (unresolvable tree, which must FAIL TOWARD ALERTING) prove the fix did not replace a false alarm with a dead one — deleting the comparison fails S2, and replacing its guard with `if true` fails S3 and S4, both OBSERVED red before restore. An exit-code sweep pins that every path still exits 0, because the 2026-06-21 incident showed a red check on main HEAD makes Railway skip the very deploy the probe verifies. \u26a0\ufe0f DECLARED LIMIT: it proves the probe's SHELL under injected inputs, not that Railway's watch path behaves as inferred — the Railway MCP returned Unauthorized and no watch path was read" },
  { path: "src/__tests__/contracts/ci-mypy-strict-surface.contract.test.ts", batch: "164.6.1 / MYPY-MAINPY-01", invariant: "the mypy --strict invocation in the ci.yml `python` job (step `Type gate - mypy strict over the running-service surface`) is pinned, by EXTRACTION rather than grep, to EQUAL the service surface derived from disk (every top-level analytics-service/ directory holding a TRACKED .py anywhere beneath it, read from git ls-files and including namespace packages without __init__.py, minus the hand-typed EXCLUDED {tests, scripts}, plus every tracked top-level *.py) and the Makefile `typecheck` set, with exactly ONE mypy invocation in the job (mypy or python -m mypy at a command start, --strict or not, backslash continuations joined, name: lines and quoted strings ignored; wrappers such as uv run are NOT counted) whose flag set is EXACTLY --strict --follow-imports=silent --config-file=pyproject.toml, a Makefile recipe whose flag set is EXACTLY --config-file=pyproject.toml, no shell operator, argfile or # token on either line, exactly one MYPY = $(VENV)/bin/mypy and one typecheck rule in the Makefile (whose ci: target must depend on typecheck), on a step that carries only name and one literal run: line (a positive rule, so quoted or spaced keys are refused) with no folded continuation line, no job-level continue-on-error of any value and no defaults shell: in block or flow form, under a pyproject.toml [tool.mypy] pinned to exactly python_version/strict=true/follow_imports=silent with overrides naming exactly the third-party set and no ignore_errors, no mypy config in any other shape, a narrow reader that refuses multi-line strings and line continuations and reads multi-line arrays, no tracked mypy.ini/.mypy.ini, no tracked .pyi stub replacing a surface module, and no inline # mypy: or top-of-file # type: ignore in a surface file — so a new top-level module or package added without widening the gate is RED. WHY: the step's comment claimed ALL running-service code while main.py, the `uvicorn main:app` entry, was off the command line and main_worker.py was followed with its errors suppressed. Calibration legs (a)–(z), (a2), (e2)–(e5), (h2)–(h4), (k2)–(k4), (l2)–(l3), (m2)–(m4), (n2)–(n3), (o2)–(o3), (s2), (t2), (u2)–(u5), (v2), (w2), (x2)–(x4), (y2), (z2), (g2), (i2) and (sh)–(sh3) on mutated in-memory copies, including an added --exclude= that leaves the path set equal and the HISTORICAL pre-phase command naming all four top-level modules, plus two non-vacuity controls. ⚠️ DECLARED LIMITS: it proves the command NAMES the surface, not that mypy is green (the `python` job does), and the CI-side RED observation is D-06b, OPEN until the phase PR; it does not pin which mypy binary runs (GITHUB_PATH, a job or workflow env PATH, a repointed VENV), nor the python job's own if: or a job- or workflow-level env: (a skipped python job is caught downstream by test-db-drift needing python and the frontend aggregator, except on a fork PR or a workflow_dispatch), and the invocation count does not see a mypy behind a wrapper word (uv run, timeout, if mypy, a quoted binary path)" },
];
...
describe("[B25] contracts registry — by-construction invariant guards", () => {
  it("registers a non-trivial set (fail-loud on accidental truncation)", () => {
    ...
    //
    // ── 164.6.1 / MYPY-MAINPY-01 — RAISED 60 → 61. One guard registered, one
    // step. The 60 baseline is MEASURED, not re-derived: the paragraph above
    // stops at 59, while the literal already stood at 60 when this phase began
    // (DEPLOYVERIFY-SHA-NOT-CODE registered its row and moved the literal
    // without adding a paragraph here).
    //   $ grep -c '^  { path:' src/__tests__/contracts/contracts-registry.test.ts
    //   60                                   # BEFORE this phase's edit, against a floor of 60
    //   61                                   # AFTER — one row added by this phase
    // The row: `ci-mypy-strict-surface.contract.test.ts`, the extraction pin
    // for the mypy --strict invocation's path set against the on-disk surface.
    //
