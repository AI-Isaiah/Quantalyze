...
const CONTRACT_GUARDS: Guard[] = [
  ...
  // ─────────────────────────────────────────────────────────────────────────
  { path: "src/lib/seam-venue-vocabulary.invariant.test.ts", batch: "140.5-02 / SEAMPROSE-05", invariant: "the Python EMITTER's venue error_code vocabulary agrees with the TS disposition table `VENUE_WIRE_CODE_TO_VERDICT` (`wizardErrors.ts`): the emitted codes are DERIVED from `analytics-service/services/exchange.py` (comment-stripped) and compared as a SET against the hand-typed dispositions, so a new emitted code fails BY NAME and a departed one reds too. ⚠️ DECLARED BLIND SPOT, asserted not narrated: `csv_adapter.py`'s dynamic `error_code = first_rule.upper()` family cannot be statically enumerated (pandera rule names read at runtime), so the 8 literal-less emitter sites are pinned by location — a SECOND dynamic emitter appearing anywhere reds instead of silently shrinking the population. Their disposition is by-family (CSV-surface codes render through `CsvValidationEnvelope`, never reaching the classifier). Registered by 140.5-08" },
