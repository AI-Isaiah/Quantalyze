...
const CONTRACT_GUARDS: Guard[] = [
  ...
  { path: "src/lib/seam-budgets.invariant.test.ts", batch: "140.2", invariant: "SEAM_ROUTE_BUDGETS row CONTENTS deep-compared to a hand-typed 15-row map (catches a dropped leg from a multi-leg row, which SC-4a/SC-4b cannot see) + on-disk maxDuration parity + the 3-path SEAM_EXCLUSIONS set + the A-12 warmer-exclusion source scan (no excluded path may import or call the core) with both /health warmers pinned as MEMBERS" },
