...
describe("PR-A / phase-19-shim-step-a — strategy_verifications upsert (C-5)", () => {
  it("upserts a complete strategy_verifications row with all 5 NOT NULL fields populated", async () => {
    ...
    // PR-X4: was 'validated'. The public-status route at
  ...
  /**
   * PR-X4 regression — the SV upsert MUST populate metrics_snapshot with
   * the `results` blob returned by the Python `/api/verify-strategy`
   * endpoint. Without this, the public-status route at
   * verify-strategy/[id]/status/route.ts returns `{status: 'published'}`
   * with NO `results` field — teaser users see "verified" but no score.
   */
  it("upserts metrics_snapshot from the Python results blob (PR-X4)", async () => {
  ...
  /**
   * PR-X4 edge case — when Python returns a verification_id without a
   * `results` blob (shouldn't happen post-PR-X2, but defensively), the
   * upsert sets metrics_snapshot to null instead of crashing.
   */
  it("upserts metrics_snapshot=null when Python returns no results blob (PR-X4)", async () => {
    verifyStrategyMock.mockResolvedValueOnce({
      verification_id: TEST_VERIFICATION_ID,
