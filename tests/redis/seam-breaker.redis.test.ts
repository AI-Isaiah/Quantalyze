 ...
 * spans it; 90 → 100 with the 153.4 review's WR-01, which found that 90 spanned
 ...
 * Hand-typed here, never imported: a lane that read the core's own constant
 * could not disagree with it.
 */
const LOCK_TOMBSTONE_S = 100;
...
describe("seam breaker against a REAL Redis (SEAMCORE-09)", () => {
  ...
  it("R-3 trip idempotency — a later trip does NOT ratchet the first writer's expiry", async () => {
    ...
    // cooldown + tombstone = 130 s, all three hand-typed. This is the number the
    ...
    expect(ttlAtTrip).toBeGreaterThanOrEqual(129);
    expect(ttlAtTrip).toBeLessThanOrEqual(130);
    expect(COOLDOWN_S + LOCK_TOMBSTONE_S).toBe(130);
  ...
  it("R-4 the ENCODED cooldown is the lock's life, the key outlives it, and Retry-After agrees", async () => {
    ...
    expect(ttl).toBeGreaterThanOrEqual(129);
  ...
  it("R-7 [SLOW, ~2 min] recovery latency is ALIGNMENT-DEPENDENT, not the flat 30 s the docblock claims", async () => {
    ...
    // tombstone runs to 130 s) and the circuit must nonetheless read CLOSED,
    ...
    // 130 s. So the same claim is now made two ways, and both are STRICTER than
