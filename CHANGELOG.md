...
**phase-19 — walk back the unified backbone for the teaser flow (PR-X4).** Two abortive 2026-05-14 kill-switch flips exposed that the unified `/process-key` pipeline has no branch that fits the teaser submission: PR-X3 set `step='validate'` to clear the `MISSING_STRATEGY_ID` gate, but the resulting `_run_validate_only` path returns no `verification_id` and the TS handler 502s. The unified backbone was designed around the wizard's multi-step `validate → finalize` pattern; the teaser is a one-shot synchronous probe with no caller-owned `strategy_id` and no follow-up. PR-X4 stops shoehorning the teaser into the unified shape and walks the route back to always run the legacy `verifyStrategy()` path. The kill-switch still gates the wizard flows (onboard / resync / csv) that legitimately go through `/process-key`. As a side effect, PR-X4 also closes the pre-existing gap where the SV row created on the teaser path got `status='validated'` with no `metrics_snapshot` — meaning teaser users polling `/api/verify-strategy/[id]/status` never saw their score. The upsert now writes `status='published'` + `metrics_snapshot` from the Python `results` blob in one shot.

### Changed

- **`src/app/api/verify-strategy/route.ts`** — removed the `unifiedVerifyStrategyHandler`, the `isUnifiedBackboneActive()` branch, and the `postProcessKey` + `isUnifiedBackboneActive` imports. The route now always runs `teaserVerifyStrategyHandler` (renamed from `legacyVerifyStrategyHandler` since "legacy" no longer fits). The handler's SV upsert writes `status='published'` (was `'validated'`) + `metrics_snapshot: { ...analyticsResult.results, matched_strategy_id }` (was: no metrics column). The `[id]/status` route at line 107 already accepts both `'complete'` (legacy VR shape) and `'published'` (canonical SV terminal) as terminal states, so the user-facing response includes the metrics blob immediately on poll.
- **`tests/integration/phase-19-pra-write.test.ts`** — flipped the status assertion from `'validated'` to `'published'`. Added 2 new tests: `upserts metrics_snapshot from the Python results blob (PR-X4)` asserting the full `{twr, sharpe, return_24h, return_mtd, return_ytd, equity_curve, trade_count, matched_strategy_id}` snapshot lands; `upserts metrics_snapshot=null when Python returns no results blob (PR-X4)` as the defensive edge case. Both verified meaningful by stripping the fix lines and confirming the tests fail.
- **`tests/integration/process-key-thin-adapters.test.ts`** — removed 4 obsolete unified-path tests (`flow_type=teaser, source from body.exchange`, `step='validate' (PR-X3)`, `X-User-Id='public' (CT-4)`, `mints public_token + expires_at (CT-3)`) and the `I-T3a: verify-strategy missing token → 503` test — all of which exercised a code path that no longer exists. Rewrote `I-T2a` from "flag=off does NOT call /process-key" to "verify-strategy NEVER calls /process-key regardless of flag (PR-X4)" so the walk-back invariant is enforced. Swapped the CT-7 `UPSTREAM_TIMEOUT` test to import `keys/sync/route` (which still rides `postProcessKey`) instead of `verify-strategy/route`. Net -211 lines.

### Investigated and root-caused

The architectural mismatch is documented in the new TSDoc at `teaserVerifyStrategyHandler` and in the test comments. The unified `/process-key`'s no-`strategy_id` dispatch (`process_key.py:503-575`) has three branches: `step='validate' → _run_validate_only` (no DB persist, no verification_id), `flow_type='csv' AND step='finalize' → finalize_csv_strategy RPC` (returns strategy_id, not verification_id), and else `422 MISSING_STRATEGY_ID`. None match the teaser semantics of "validate keys → fetch trades → compute metrics → persist a verification row → return verification_id." Adding a 4th teaser-specific branch was considered (option A in the investigation report) but rejected as the wrong architectural direction — the teaser flow does not belong inside a wizard-shaped pipeline. PR-D (BACKBONE-04 step (d)) is unaffected: it ships migration 107 which renames `verification_requests` to a VIEW; the teaser route never wrote there post-PR-X2 and now writes to `strategy_verifications` directly with metrics, so the migration applies cleanly and the kill-switch goes back to gating the wizard surface only.
