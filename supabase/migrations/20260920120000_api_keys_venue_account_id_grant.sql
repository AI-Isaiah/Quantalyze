...
--
-- ⭐ RESOLVED 2026-09-20 — THE SPLIT WAS CHOSEN, AND THIS FILE IS THE FIRST PR.
-- Founder decision at ship time. This PR carries the GRANT and NOTHING that
-- reads it: no constants.ts, no types.ts, no component, no route. The
-- constants.ts/types.ts projection change and the rest of Phase 164.5.3 ship
-- in a SECOND PR, after this one is applied and confirmed green on PROD.
--
-- ⭐ WHY THE SPLIT RATHER THAN CAREFUL SEQUENCING: the hazard above is not an
-- edge case on a single-PR merge, it is the DEFAULT OUTCOME. Measured
-- 2026-09-20: vercel.json declares no ignoreCommand and no build gate, so the
-- Vercel promotion starts on the push to `main` and finishes in minutes, while
-- PROD's `apply` sits behind the `Production` environment's HUMAN reviewer
-- gate. The frontend winning that race is what happens unless someone is
-- standing at the keyboard. Splitting removes the race instead of documenting
-- it.
--
-- ⭐ THE EXISTING DRIFT GATE CONFIRMS THIS ORDER, it does not merely tolerate
-- it. `src/lib/sec-005-live-probe.test.ts`'s "API_KEY_USER_COLUMNS matches the
-- live GRANT — no drift" arm PROJECTS the roster and fails when a rostered
-- column is not granted — roster ⊆ GRANT, one-way. A GRANT that is ahead of
-- the roster (this PR) passes; a roster ahead of the GRANT (the reverse order)
-- is exactly what that arm catches.
--
