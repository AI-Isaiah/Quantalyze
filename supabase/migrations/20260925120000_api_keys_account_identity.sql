...
COMMENT ON FUNCTION public.set_departed_key_history_inclusion(uuid, text) IS
  ...
  'reclaims one that has run for more than 10 minutes). The same 55006 '
  'HISTORY_RECOMPOSE_IN_PROGRESS, with a different DETAIL, answers a pending '
  'recompose another request queued while the reused row was being put back '
  'to pending (a retry folds into it). The job is judged twice: before the '
