...
COMMENT ON FUNCTION public.set_departed_key_history_inclusion(uuid, text) IS
  ...
  'reclaims one that has run for more than 10 minutes). The running test is '
  'made twice: before the write, and again on the job the enqueue returns, '
  'which is locked FOR UPDATE, so a job enqueued and claimed by another '
  'transaction in between is refused too. Every pending, failed_retry, '
