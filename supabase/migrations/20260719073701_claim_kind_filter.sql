-- ==========================================================================
-- FLIPRETRY-02 (Phase 123): kind-filtered claim RPC
-- ==========================================================================
-- Re-bases the FULL latest body of claim_compute_jobs_with_priority
-- (migration 20260603120000_compute_jobs_rpc_clear_error_and_gin_fanin.sql
-- :170-341) and adds two OPTIONAL kind-filter args:
--     p_kind_include TEXT[] DEFAULT NULL
--     p_kind_exclude TEXT[] DEFAULT NULL
--
-- WHY: the v1.11 derived-allocator-curve backfill wedged the SEQUENTIAL prod
-- worker — one slow/uncancellable live-exchange crawl blocked the shared loop
-- event-loop, healthz went stale ~12min, and the 90s auto-restart never fired.
-- Per-crawl wait_for alone cannot keep the shared loop's healthz fresh (5
-- sequential 300s-bounded jobs = 25 min with LAST_TICK_AT frozen). The only
-- structural guarantee is ISOLATION: a DEDICATED backfill worker claims ONLY
-- the backfill kinds and the prod worker EXCLUDES them, so a non-yielding
-- backfill crawl can never freeze the interactive loop.
--
-- Filter semantics (both applied to EVERY kind-touching read in the body — the
-- v_high_pending throttle probe AND the `ranked` claim SELECT — so a filtered
-- worker's low-priority throttle decision is scoped to the kinds it can
-- actually claim; an include-filtered backfill worker must NOT defer on
-- unrelated prod normal/high pending it will never claim):
--     (p_kind_include IS NULL OR kind = ANY(p_kind_include))
--     AND (p_kind_exclude IS NULL OR NOT (kind = ANY(p_kind_exclude)))
-- Both NULL => byte-identical behavior to the 20260603120000 definition
-- (proven behavior-identical by supabase/tests/test_claim_kind_filter.sql).
--
-- DROP-then-CREATE (NOT CREATE OR REPLACE): a different arg list would create
-- an OVERLOAD, and the worker's 3-named-arg PostgREST rpc() call would then be
-- AMBIGUOUS. The drop/create window is deploy-safe: main_worker's
-- structured-42883 fallback latch (main_worker.py :415-485) degrades to the
-- legacy 2-arg claim_compute_jobs for that tick and self-heals on re-probe.
--
-- Re-base rule: this migration adds ONLY the two filter predicates. Every
-- fencing / priority / dedupe / metadata-stamp guarantee of the latest body is
-- preserved verbatim (STEP 3 of the sibling gate + the two CI signature tests
-- assert this). SECURITY DEFINER + `SET search_path = public, pg_temp` and the
-- REVOKE FROM PUBLIC/anon/authenticated are re-stated for the new signature
-- (the old COMMENT/REVOKE die with the DROP).
-- ==========================================================================
...
COMMENT ON FUNCTION claim_compute_jobs_with_priority(INTEGER, TEXT, BOOLEAN, TEXT[], TEXT[]) IS
  ...
  'failed_retry candidacy and C39 done_pending_children guard + M-1137/M-1138 '
  'last_error/error_kind clear on re-claim. FLIPRETRY-02 (Phase 123): adds '
  'p_kind_include / p_kind_exclude (both DEFAULT NULL => byte-identical to the '
  '3-arg prod behavior) applied to BOTH the v_high_pending throttle probe AND '
  'the claim SELECT so a dedicated backfill worker claims ONLY backfill kinds '
  'and the interactive prod worker excludes them. All pre-existing guarantees '
  'preserved verbatim (asserted by the two signature-pinned CI SQL gates).';
