...
--
-- STATE-ADAPTIVE (Phase 156 precedent). The shared TEST database receives this
-- migration only AFTER the PR merges, but this test runs on the PR. So the test
-- reads pg_get_functiondef first: on a pre-migration database it RAISE NOTICEs
-- and skips every assertion. A MISSING FUNCTION is still a hard failure (the RPC
-- has existed since 20260626120000) — only the un-applied GATE is skippable.
--   ⚠️ Consequence, stated honestly: until the first CI run AFTER this migration
--   reaches TEST, this file has never been observed ARMED and green. Until then
--   the correct phrasing is "would have caught", never "did catch".
--
...
BEGIN
  ...
  IF position(v_gate_pred IN v_def) = 0 THEN
    RAISE NOTICE 'test_get_verified_cohort_rank_gate: SKIPPED — the deployed get_verified_cohort_rank does not yet carry the RANK-01 computed-analytics gate. This database predates migration 20260821120000; the shared TEST project receives it only after merge. This is the designed pre-migration state, NOT a pass: no assertion below has been evaluated.';
    RETURN;
  END IF;
  ...
  -- (2) SECURITY DEFINER + pinned search_path survived
