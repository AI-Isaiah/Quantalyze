...
-- function matches the canonical (p_strategy_id uuid, p_kinds jsonb) signature. A future
...
BEGIN
  SELECT COUNT(*),
   ...
   WHERE n.nspname = 'public'
     ...
     AND pg_get_function_identity_arguments(p.oid) = 'p_strategy_id uuid, p_kinds jsonb';

  IF match_count = 0 THEN
    RAISE EXCEPTION
      'H-0762 Test 1: upsert_strategy_analytics_series_batch(p_strategy_id uuid, p_kinds jsonb) does not exist (overload drift?)';
  END IF;
  IF match_count > 1 THEN
    RAISE EXCEPTION
      'H-0762 Test 1: upsert_strategy_analytics_series_batch(p_strategy_id uuid, p_kinds jsonb) matched % rows — schema corruption',
      match_count;
  END IF;
