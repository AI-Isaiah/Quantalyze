...
- **⚠️ USER-FACING — SEAMUX-03: 9 of 15 seam-importing routes still emit bare `{ error: … }`, not the typed `{ code: … }` error envelope.** Independently re-confirmed OPEN at HEAD; not measured by any of 140.3's 7 success criteria (green criteria table ≠ requirement delivered). Zero-`code` routes: `verify-strategy` (11 bare shapes), `admin/match/eval` (6), `admin/match/recompute` (9), `scenario/optimize` (12), `simulator` (8), `bridge` (7), `portfolio-optimizer` (5), `keys/validate-and-encrypt` (10), `strategies/csv-validate` (1). REQUIREMENTS.md explicitly names the public teaser, the CSV routes, and the admin match routes. Owed: an arm-by-arm pass in the shape 140.3-10 used on keys/sync.
- **⚠️ USER-FACING — SEAM_MISCONFIGURED renders as `UNKNOWN` on the two wizard clients** (140.4). `ConnectKeyStep.tsx:392` (checks `KNOWN_CREATE_WITH_KEY_CODES`, no `SEAM_MISCONFIGURED` member / no `recogniseSeamErrorCode` hop; `create-with-key/route.ts:242` emits it) and `MultiKeyConnectStep.tsx:727` (checks `KNOWN_ADD_KEY_CODES`; `composite/add-key/route.ts:254` emits it) both collapse the code to `UNKNOWN` — "Try the last action again." — for a fault whose authored copy says "Retrying will not clear it." Two plans (140.4-12/-13) each believed the other owned the roster edit; neither closed it. Owed: a translate-first hop at `ConnectKeyStep.tsx:391` + `MultiKeyConnectStep.tsx:726` matching `SyncPreviewStep.tsx:670-675`, plus a named test per surface.
