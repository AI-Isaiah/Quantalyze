...
- **Per-phase PRs cannot pass CI independently (accepted, founder-sanctioned).** Slicing the interdependent branch splits some tests from their subjects across cut boundaries. Confirmed straddle: `test_validate_key_venue_transient.py` fails on every pre-140.5 cut ("no `/api/validate-key`/`/api/verify-strategy` route on `main.app`" — the `/api`-prefix wiring lands in 140.5) but is **23/23 at the tip**. `sql-tests` straddled on 140.1–140.3, green from 140.4. **Acceptance rule per intermediate PR:** merge only if the ONLY red jobs are `python`/`sql-tests`/`frontend-test` attributable to a straddle or the isolation flake AND every gate job (`frontend-build`, `frontend-typecheck`, `frontend-lint`, `contracts`, `e2e`, `e2e-seeded`, `policy`, `knip`, `secret-scan`, `snapshot-drift`) is green. This PR (140.5, full tip) is the real gate and is expected fully green.
