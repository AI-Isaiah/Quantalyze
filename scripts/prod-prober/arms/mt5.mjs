...
export const REMEDIES = {
  ...
  "mt5-ipc-timeout":
    "The bridge IS ATTACHED but the terminal is NOT ANSWERING (-10005). Two known causes read identically on this transcript, and it cannot tell them apart: the modal-login-dialog wedge, PERSISTED with the Wine prefix on the named volume and unaffected by a redeploy; and the account-switch wedge measured 2026-09-21, a PROCESS state where the terminal's own console was clean and the Alerts tab was empty even though it was not answering. Do this in order: first, recycle the gateway's terminal process. The analytics service's MT5 session monitor attempts that recycle automatically when its own poll reads -10005 (`_escalate_ipc_fault` in analytics-service/services/mt5_relogin.py, polling every MT5_SESSION_POLL_INTERVAL_S), so read its log for the verdict before acting by hand. A recycle is unattended and cannot make the dialog cause worse, but it is not instant. Measured: 2.0 s to authorized for the 2026-09-21 manual restart; on the 2026-09-25 kill-and-relaunch spike, 86 s from the kill to logged in (run 1), and about 4m45s to a relaunched terminal when the relaunch waited for the next caller (run 2). If -10005 returns, open the gateway's VNC console and clear the MODAL LOGIN DIALOG by completing any login; a redeploy does NOT fix that cause, because the dialog and the Wine prefix live on the persistent volume and come straight back. See the -10005 differential-diagnosis section in docs/runbooks/mt5-go-live.md for the full procedure. A transient reading is possible while a real validate call holds the terminal's IPC bridge, so a SECOND consecutive hourly hit is the confirmation.",
  "mt5-ssh-transport":
