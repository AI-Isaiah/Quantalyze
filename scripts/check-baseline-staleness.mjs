 ...
 *
 * @param {{recordedSha: string|null, actualSha: string|null, actualBytes: number}} facts
 ...
 */
export function judge({ recordedSha, actualSha, actualBytes }) {
  const defects = [];
...
function selfTest() {
  ...
  console.log("=== SELF-TEST 1/8: recorded hash equals the file's hash → clean");
  ok(judge(clean).length === 0, "no defects when BASELINE.md and baseline.sql agree");

  console.log("=== SELF-TEST 2/8: the dump moved, the provenance row did not (the CO-EDIT defect)");
  ...
  console.log("=== SELF-TEST 3/8: BASELINE.md carries no parseable sha256 row (D-02)");
  ok(
  ...
  console.log("=== SELF-TEST 4/8: an absent row and a wrong row are DISTINGUISHABLE");
  ...
  console.log("=== SELF-TEST 5/8: baseline.sql unreadable → its own kind, never a pass");
  ok(
  ...
  console.log("=== SELF-TEST 6/8: baseline.sql present but EMPTY → same kind, still not a pass");
  ...
  console.log("=== SELF-TEST 7/8: an unmeasurable dump is never reported clean");
  ...
  console.log(`=== SELF-TEST PASSED: ${asserted}/${asserted} assertions, every defect kind fired on its own input ===`);
  return 0;
}
...
function main(argv) {
  ...
  const defects = judge({ recordedSha, actualSha, actualBytes });
