...
/**
 * Pending plans: `*-PLAN.md` under `.planning/phases/*​/` with NO sibling
 * `*-SUMMARY.md`. An executed plan is archival — its anchors describe a tree
 * that has legitimately moved on, and scanning it would pin the corpus to a
 * frozen past.
 */
export function findPendingPlans(root) {
  ...
  if (!existsSync(phasesDir)) {
    return {
      ...
      pending: [],
      measureFail: true,
      measureReason: `${relative(root, phasesDir)} does not exist — the plan corpus could not be located at all.`,
    };
  }

  let planFiles = 0;
  const pending = [];
  for (const dir of readdirSync(phasesDir, { withFileTypes: true })) {
    ...
    for (const name of readdirSync(abs)) {
      ...
      pending.push(join(".planning", "phases", dir.name, name).split("\\").join("/"));
    }
  }
  pending.sort();
  ...
  if (planFiles === 0) {
    return {
      ...
      measureFail: true,
      measureReason:
        ".planning/phases contains NO *-PLAN.md files. Zero pending plans because zero plans were found is a broken glob, not a clean corpus.",
    };
  }

  return { planFiles, pending, measureFail: false, measureReason: null };
}
...
function main(argv) {
  ...
  if (args.includes("--pending")) {
    ...
    if (found.pending.length === 0) {
      // A genuine measurement, not a broken glob: plan files exist and every
      // one of them has been executed.
      console.log("claims: 0 checked");
      console.log("OK: 0 plan file(s), no stale claims. Every plan in the tree has a SUMMARY.");
      return 0;
    }
