...
lint *errors* on the clean `src/lib/design-tokens/**` token surface plus the
Phase-52 allocator-journey surfaces that have been migrated and grep-verified
clean (`compare/**`, `discovery/**`, `strategy/[id]/**`, `CompareTable.tsx`,
`StrategyGrid.tsx`, and the clean per-file allocations/factsheet-v2 list in
`eslint.config.mjs`). Across the rest of `src/**` it stays a non-blocking `warn`.
The remaining surfaces ratchet to `error` one at a time as phases 53/54 migrate
them, so DS-04's raw-px rejection is a per-surface ratchet (a hard backstop on
the proven-clean surfaces, a visible warn elsewhere), not yet an app-wide
prohibition.
