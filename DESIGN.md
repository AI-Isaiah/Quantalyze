...
- **Measure ladder (max content width):** three measures, picked by content
  ...
  document** (the default factsheet / single-strategy document width), and
  **FLUID dense tables** (ultra-wide allocator tables, compare grids,
  My Strategies — `max-w-full`, **no px ceiling**). Wider content earns a wider
  measure; prose never exceeds 1100px regardless of viewport.
  - ⭐ **Dense tables carry NO cap** (founder decision 2026-08-09, superseding
    the Phase 52 `max-w-[1920px]` rung). A fixed px cap turns surplus viewport
    into dead margin precisely when the user zooms out to see *more* — at 50%
...
| 2026-07-16 | Sidebar navy #0F172A → light rail | Founder disliked the flat navy rail; chose the light-surface direction, deliberately reopening the previously-locked "keep current" token. Nav now: light `bg-surface`, right hairline (`border-r border-border`), mono-eyebrow section headings (uppercase Geist Mono, tracked — the factsheet annotation voice), accent-text active state, ACCOUNT pinned to the bottom. The navy is no longer used by the nav; the `--color-sidebar*` tokens are NOT yet deleted — one non-nav consumer (`ApiKeyManager` exchange-avatar tint, `bg-sidebar/10`) still references them, so their removal is a follow-up once that chip is migrated to a neutral token. || 2026-08-09 | Dense tables lose their px cap — measure ladder rung 3 becomes FLUID (founder decision) | Founder report with screenshots: *"if I zoom out with ctrl -, then the space between the menu and the fields gets larger… only fonts and pictures should shrink. The content itself should still go over the entire page, as zooming out should allow me to see more of the content. It should never produce dead/empty areas."* The surface hit was **My Strategies (`/my-strategies`)**, and the mechanism was a **two-owner width bug**: the page set `mx-auto max-w-[1920px]` under a comment asserting "(dashboard)/layout.tsx does NOT cap width", which was false — `DashboardChrome` caps every route at `max-w-7xl` (1280px) unless it is on the `isWide` allow-list, and `/my-strategies` had never been added. So the page's own cap was dead code, the table rendered 640px narrower than intended, and it showed "Scroll for more columns →" beside empty space. **Options weighed (TODOS.md):** (A) keep the ladder, accept dead space; (B) fluid for DATA surfaces only, bounded measure kept for prose/forms; (C) no caps anywhere. **Founder chose (B).** Rationale: any FIXED px ceiling dead-spaces once the viewport exceeds it, and zooming out is exactly how a viewport exceeds it — at 50% zoom a 2560px display reports 5120 CSS px, leaving ~3200px stranded even at the 1920px rung. Data is the one content type where more width is strictly more information; prose is not, so 1100px stays (a 5120px-wide paragraph is measurably harder to read — the eye loses the line return). Per Rule 7 the conflict was surfaced and one side picked, **not blended into a compromise cap**. **Changes:** shell `isWide` → `max-w-full` and gains `my-strategies`; the page-level `max-w-[1920px]` caps on `/my-strategies`, `/allocations` (+loading), `/compare` (+loading) and `/discovery/[slug]` are **deleted** so the shell is the sole owner (leaving them would re-clamp the fluid shell and silently restore the bug). `/strategies` (legacy card list), `/strategies/new/wizard` (form) and `/strategies/[id]` (1440px document) are unaffected; the four RT-W2 admin prose pages keep 1100px. **Guard note:** the shell test that asserted `/strategies` stays narrow — reasoning "/strategies (incl. the new-strategy wizard) is a form surface" — had encoded the list-vs-wizard conflation as an expectation; it was replaced, and the new rows reject the *arbitrary-value form* `max-w-[Npx]` rather than pinning a successor literal, so a reinstated ceiling of any size reds. |
