...
| 46 — Surface-by-Surface Reflow (CSS-first, no charts) | 4/4 | Complete   | 2026-06-27 |
| 47 — Hand-Rolled SVG Charts (touch + legibility + portrait) | 5/5 | Complete    | 2026-06-28 |
| 48 — Recharts + EquityChart + Final Verification | 5/5 | Complete   | 2026-06-28 |
...
- [x] 46-03-PLAN.md — Wizard de-block: delete DesktopGate + test, render Suspense directly (auth gate + boundary preserved), CSS-first layout reflow, coverage ratchet measured [WIZARD-01]
- [x] 46-04-PLAN.md — Parametrized reflow sweep (public unseeded + seeded authed + degenerate route) + honest-state verify + FLOW-01 ci.yml dual-wiring [REFLOW-01, REFLOW-02, REFLOW-03]
...
- [x] 47-01-PLAN.md — Shared `useTapPin` tap-vs-drag + pin-toggle gesture hook (extracted from TimeSeriesChart) + branch tests
- [x] 47-02-PLAN.md — Legibility + portrait for the 5 no-hover factsheet SVG panels (Distribution/Signature/CrossSignature/Histogram/MasterBrush)
- [x] 47-04-PLAN.md — Standalone charts: ReturnQuantiles + Sparkline + MonteCarloBandChart legibility/portrait + MCB Vitest desktop-byte-identity snapshot

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 47-03-PLAN.md — Touch tap-reveal/pin for the 3 charts with a real desktop hover (Streak/DailyReturnsHeatmap/DailyHeatmap) + their legibility/portrait

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 47-05-PLAN.md — Fresh seeded svg-chart-parity spec (desktop goldens + 320px portrait) + ≥44px chart tap-rect gate + FLOW-01 ci.yml wiring + ratchet
...
**Plans**: 5 plans (Wave 0 install/stubs 48-01; Wave 1 chart swaps 48-02 + EquityChart 48-03 file-disjoint parallel; Wave 2 axe matrix + FLOW-01 wiring 48-04; Wave 3 lhci budget + HUMAN-UAT 48-05)
Plans:
**Wave 1**

- [x] 48-01-PLAN.md — Wave 0: install @lhci/cli@0.15.1 (pinned, legitimacy-gated) + TouchTooltip shim (both-branch tested) + failing/skipping stubs (axe-app-wide, lighthouserc, EquityChart touch test) [CHART-01b, A11Y-01, A11Y-03]
- [x] 48-02-PLAN.md — Wave 1: swap all 18 tooltip-bearing Recharts charts to TouchTooltip (desktop byte-identical, OutcomesWidget parity-only, accessibilityLayer grep green) [CHART-01b]
- [x] 48-03-PLAN.md — Wave 1: additive useTapPin touch path on EquityChart (pointerToIndex == handleMove math, desktop mouse path byte-identical, ResizeObserver/projection-memo untouched, >=44px coarse hit) [CHART-01b]

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 48-04-PLAN.md — Wave 2: app-wide axe route x viewport matrix (embedded serious+critical scoped) + target-size EquityChart >=44px case + FLOW-01 dual-wire + bespoke-matrix-confirm [A11Y-01, CHART-01b]

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 48-05-PLAN.md — Wave 3: @lhci/cli mobile perf budget (baseline-seeded, public-only, lighthouse-mobile job) + rotate-stability fold + 48-HUMAN-UAT.md + final ratchet/guards verification (human_needed) [A11Y-03]
...
---
*Last updated: 2026-06-28 — Phase 48 planned (5 plans: Wave 0 install+stubs 48-01; Wave 1 Recharts swaps 48-02 + EquityChart touch 48-03 file-disjoint parallel; Wave 2 app-wide axe matrix + FLOW-01 wiring 48-04; Wave 3 lhci mobile perf budget + HUMAN-UAT 48-05. All 3 REQ-IDs covered: CHART-01b/A11Y-01/A11Y-03; verification ends human_needed for the real-device authed sign-off).*
