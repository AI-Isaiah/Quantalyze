...
- [x] **Phase 27: Forward Uncertainty (Monte-Carlo Bands)** - Block-bootstrap forward confidence bands / return distribution, honest to sample size, gated below the minimum-sample floor (built 2026-06-22, shipping)
- [x] **Phase 28: Weight Optimizer** - Python analytics-service min-vol / max-Sharpe solver (Ledoit-Wolf shrinkage, long-only, write-to-draft-only) with TS↔Python parity pinned (completed 2026-06-22)
...
**Plans**: 2 plans in 2 waves
  - [x] 27-01-PLAN.md — pure seedable block-bootstrap MC lib (`scenario-montecarlo.ts`) over `portfolio_daily_returns` + linear parameter-uncertainty drift (honest-to-N) + thin Web Worker glue + 19 golden/determinism/floor/calibration tests [wave 1]
  - [x] 27-02-PLAN.md — `MonteCarloBandChart` (SVG fan) + `MonteCarloSection` (off-thread worker lifecycle + 5-state routing) + composer mount + `worker-src` CSP + 16 state-matrix tests [wave 2]
**UI hint**: yes
...
| 27. Forward Uncertainty (Monte-Carlo Bands) | 2/2 | Complete (shipping) | 2026-06-22 |
