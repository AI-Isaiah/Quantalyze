...
| `isValidDollar` / `MAGNITUDE_CAPS` | `src/app/api/strategies/finalize-wizard/route.ts:404-409` | dollar caps — TWO DISTINCT bounds (corrected rev-4, review round 3 W-3): the allocation ticket → `MAX_TICKET_SIZE_USD` ($1e9 — the "$1B sanity cap" copy's literal bound); AUM / max capacity → `MAX_DOLLAR_VALUE_USD` ($1e12) | Already the repo's server-side dollar validator. Currently route-local — lift to a shared module rather than minting a second validator. [VERIFIED: grep] |
...
| $1B sanity cap | A new validator | `MAGNITUDE_CAPS.MAX_TICKET_SIZE_USD` ($1e9) for the allocation ticket; `isValidDollar` + `MAGNITUDE_CAPS.MAX_DOLLAR_VALUE_USD` ($1e12) stays the AUM/capacity bound (corrected rev-4 — review round 3 W-3) | One definition per cap today (canonical home `closed-sets.ts:529-546`); a second would drift. |
...
| V5 Input Validation | yes | `typeof` guards + trim + bounds before the write, and **before** the rate-limit token is consumed. Mark value against a closed set (`'own_capital' \| 'team_review'`) with a 400 on anything else — mirrored by the DB CHECK. Allocation amount via the `MAX_TICKET_SIZE_USD` ($1e9) ticket cap; AUM/capacity keep `isValidDollar` + `MAX_DOLLAR_VALUE_USD` ($1e12) (corrected rev-4 — review round 3 W-3). Name: non-empty after trim, ≤80, **reject not truncate**. |
