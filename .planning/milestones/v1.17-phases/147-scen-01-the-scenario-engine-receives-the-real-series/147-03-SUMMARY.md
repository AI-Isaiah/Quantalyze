...
- `node_modules/next/dist/docs/` does not exist inside the worktree; Node resolves `node_modules` by walking up to the repo root. The AGENTS.md-mandated `06-fetching-data.md` was read from `node_modules/next/dist/docs/01-app/01-getting-started/`. It confirms `await`-ing a DB client in an async Server Component is the current pattern; `cacheComponents` is not enabled in `next.config.ts` and the page is `force-dynamic`, so no `use cache`/Suspense constraint applies.
