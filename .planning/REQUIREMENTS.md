...
| WIZFORM-05 | Phase 153 (v1.17) | **PARTIAL** — 153.3 landed the server half (bounded end-to-end deadline + terminal lease); 153.4-01 minted the client `validate-key-serialized` row at 120 000 ms with `BREAKER_LOCK_TOMBSTONE_S` 60→90 (D-26) and pinned client > the 105 s server worst case; **153.4-02 made it LIVE** — `budgetKeyFor(exchange)` selects by CAPABILITY (`venueIsSerialized`, never a venue name), `validateKey` spends what it returns, and the three validate routes declare `default-venue` / `serialized-venue` as mutually exclusive `branch` legs so SC-4b takes a MAX (worst case 248 250 ms against a 300 000 ms ceiling). **153.4-03 BUILT D-05's honest long-wait UI** — `src/lib/wizard/validate-budget.ts` (a client-safe twin of both budgets, pinned to `SEAM_BUDGETS` with both sides hand-typed) plus `ValidateWaitCard` (one status label per venue CLASS, an escalation ladder keyed to FRACTIONS of the configured budget, and a non-destructive `Stop waiting` at >=40%). **153.4-04 MOUNTED IT on the single-key connect step** — an `AbortController` on the validate, a 1 s tick, the 300 ms render gate, a `Stop waiting` that aborts the in-flight POST and keeps every field, and a client deadline at budget + 15 000 ms that emits `SEAM_DEADLINE_EXCEEDED` rather than spinning forever. ⭐ **The `REQUIRES_CONNECT_SURFACE` gate is PAID**: that commit passes `surface: "connect"` and `budgetSeconds` (and `venue`, closing D-17 at this surface) in the same commit that starts emitting the code, so the "Your key details are still on this page." reassurance actually renders. **153.4-05 CLOSED THE CLIENT LEG on the composite surface** — every member panel now carries its own `AbortController`, its own frozen attempt venue, its own budget and its own `Stop waiting`, all driven from ONE step-level 1 s interval; controllers are keyed by `panel.id` so a reorder cannot redirect an abort, and the same `surface` / `budgetSeconds` / `venue` context is paid at the per-panel `buildEnvelope`. Per-panel isolation is asserted four independent ways (scope, budget, abort target, reorder) and three of them were observed to red when the mechanism was neutered. ✅ **Complete 2026-08-11 for both legs:** the server finishes inside a bounded end-to-end deadline (153.3) and the client grants 120 000 ms against a 105 s server worst case (pinned, and asserted to exceed it), so an honest verdict has room to arrive — and when it does not, the surface says so instead of spinning. ⚠️ **Two things this row does NOT claim.** D-27 leaves 120 000 ms **PROVISIONAL**: it is the founder's stated staleness tolerance, not a measurement, and **Phase 155 (MT5-VERIFY) is what proves it true against a live broker on a trading day** and tightens it from the p50/p95 153.3's instrumentation produces. And `MultiKeyConnectStep`'s own `EXCHANGES` array still offers no MT5 card, so a serialized venue reaches a composite panel only via the single-key draft carry-over (logged, Phase 155's neighbourhood) |
