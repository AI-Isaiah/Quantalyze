...
| MT5-06..10 | Phase 153 (v1.17) | Pending — re-homed from v1.16 Phase 142.3 into v1.17 (originally split out of 142.2 on 2026-08-03 at the D-14 valve); LAST by design — live funded account, real trading day, stable surface |
...
| OWN-03 | Phase 148 (v1.17) | Pending — **current behaviour now ESTABLISHED, not unverified**: portfolio correctly does NOT auto-update. Founder call — the deliverable is a **wizard question** (own-capital vs verifying-a-team), NOT an auto-add. ⚠️ first WRITING requirement in the OWN set → money-path review |
...
| MT5-14 | Phase 151 (v1.17) | Pending — re-homed from v1.16 Phase 142.3; MT5 missing from the metadata exchange chips + preselect from the connected key; ⛔ deliberate no-widening pin will red — re-cut it consciously, reasoning updated, same commit |
| MT5-15 | Phase 153 (v1.17) | Pending — ALL THREE MT5 strategies on PROD are `complete_with_warnings`; ⚠️ NOT investigated. Do not read `MT5-05 ✅` as 'the numbers are audited'. ⛔ MT5-07 does NOT close this |
| WIZCONT-01 | Phase 152 (v1.17) | Pending — **OBSERVED**: allocators have NO resume path at all (`/strategies/*` is manager-only; the overlay hardcodes `initialDraft={null}`, Phase 110 deferral) |
| WIZCONT-02 | Phase 152 (v1.17) | Pending — **LOW**; corrected 2026-08-04, the common case is already safe (localStorage session token + `strategies_user_wizard_session_source_uniq`) |
| WIZFORM-01 | Phase 151 (v1.17) | Pending — **blocking UX**; inline field errors, cost 3 failed submits and drove wrong-field edits |
| WIZFORM-02 | Phase 151 (v1.17) | Pending — code-less 400 → `UNKNOWN`; 142.2 plan 07's sweep missed this validator |
| WIZFORM-03 | Phase 151 (v1.17) | Pending — "switch to a different exchange" is impossible advice for MT5 |
| WIZFORM-04 | Phase 151 (v1.17) | Pending — **blocking UX**; transient seam timeout must not become a user decision; ⛔ ask whether the per-submit re-validation is needed before adding retries |
| STALE-01 | Phase 152 (v1.17) | Pending — root cause NOT yet established; investigate before planning |
| MT5-GOAL-01 | Phase 153 (v1.17 — umbrella acceptance gate) | **Umbrella** — no implementation work of its own; MT5 'works' only when SCEN-01 + OWN-02 close. Exists so `MT5-05 ✅` is never read as 'MT5 works' |
...
| SCEN-02 | Phase 150 (v1.17) | Pending — no ownership marker exists; ownership bit deliberately discarded at `browse/route.ts:220`. ⚠️ additive WIRE change + persisted-schema bump |
| SCEN-03 | Phase 150 (v1.17) | Pending — scenario rows are not clickable at all; ⛔ depends on OWN-02 (Phase 148) for the factsheet link |
| SCEN-04 | Phase 150 (v1.17) | Pending — row numbers carry no labels; founder could not tell what they meant |
| SCEN-05 | Phase 150 (v1.17) | Pending — two identical 'Alpha Centauri' rows in Browse; related to but distinct from WIZCONT-02 |
| AUM-01 | Phase 149 (v1.17) | ⛔ **DESIGN FLAW** — AUM is derived-only, no input anywhere; blank-slate (the primary use case) cannot size or commit. ⚠️ does NOT cause the 0.00 metrics — that is SCEN-01 |
| AUM-02 | Phase 149 (v1.17) | ⛔ **NOT a fence — a CRASH**. Holdings sync is ccxt-only; PROD `sync_error = "'Mt5Session' object has no attribute 'fetch_balance'"` in a USER-VISIBLE column. sFOX same latent bug. ~1–1.5d, medium risk (gateway concurrency) |
| AUM-03 | Phase 149 (v1.17) | ⛔ **Worse than filed** — copy says "toggle on a live holding"; that control **does not exist** in the composer, deliberately (CONSTIT-03) |
| AUM-04 | Phase 149 (v1.17) | ⛔ **ROOT CAUSE of AUM=0** — blank slate was FORCED: all-or-nothing `perKeyDailiesGateSatisfied` hides "From my book". ⭐ MANAGER-side mt5 keys pin the ALLOCATOR's gate false permanently (cross-role contamination) |
| AUM-05 | Phase 149 (v1.17) | Pending — **sFOX will crash the holdings sync identically** (`get_balances` not `fetch_balance`); invisible only because the flag is off. ⛔ close BEFORE sFOX go-live; fix the non-ccxt CLASS not the MT5 instance |
| NAV-01 | Phase 148 (v1.17) | Pending — no 'my strategies' nav entry; the allocator side is write-only. ⛔ depends on OWN-02 (same phase — strictly after it) |
...
**Coverage (v1.17 — added 2026-08-04 at roadmap creation):**
- In-scope v1.17 requirement IDs: 29 (28 work + 1 umbrella MT5-GOAL-01)
- Mapped to Phases 147–153: 29/29 ✓, each to exactly one phase — 147 SCEN-01 · 148 OWN-02/03/04 + NAV-01 ·
  149 AUM-01..05 · 150 SCEN-02..05 · 151 WIZFORM-01..04 + MT5-14 · 152 WIZCONT-01..02 + STALE-01 ·
  153 MT5-06..10 + MT5-15 + MT5-GOAL-01 (umbrella acceptance gate)
