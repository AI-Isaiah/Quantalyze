...
| WIZFORM-05 | Phase 153 (v1.17) | **PARTIAL** — 153.3 landed the server half (bounded end-to-end deadline + terminal lease); 153.4-01 minted the client `validate-key-serialized` row at 120 000 ms with `BREAKER_LOCK_TOMBSTONE_S` 60→90 (D-26) and pinned client > the 105 s server worst case; **153.4-02 made it LIVE** — `budgetKeyFor(exchange)` selects by CAPABILITY (`venueIsSerialized`, never a venue name), `validateKey` spends what it returns, and the three validate routes declare `default-venue` / `serialized-venue` as mutually exclusive `branch` legs so SC-4b takes a MAX (worst case 248 250 ms against a 300 000 ms ceiling). **153.4-03 BUILT D-05's honest long-wait UI** — `src/lib/wizard/validate-budget.ts` (a client-safe twin of both budgets, pinned to `SEAM_BUDGETS` with both sides hand-typed) plus `ValidateWaitCard` (one status label per venue CLASS, an escalation ladder keyed to FRACTIONS of the configured budget, and a non-destructive `Stop waiting` at >=40%). ⚠️ Still PARTIAL: the card is **not mounted anywhere** — 153.4-04 (`ConnectKeyStep`) and 153.4-05 (`MultiKeyConnectStep`) own the abort path, the 300 ms mount gate and the `surface: "connect"` / `budgetSeconds` that `SEAM_DEADLINE_EXCEEDED`'s `REQUIRES_CONNECT_SURFACE` bullet needs — and D-27 leaves the 120 000 ms PROVISIONAL until Phase 155 tightens it from the p50/p95 153.3's instrumentation produces. Owner: 153.4-04 / 153.4-05 |
