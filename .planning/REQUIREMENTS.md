...
| PARITY-04 | Phase 153.6 (v1.17) | 🟡 Code complete (plans 153.6-03 + 153.6-04), **NOT yet certified** — DB half: `attested_venue` + two re-based SECURITY DEFINER RPCs + SECURITY INVOKER scrub trigger + census-pinned backfill (`20260811210000`). Route half: both probe-gate arms read the attestation, no `?? exchange` fallback anywhere on the gate path. D-backfill + D-trigger mutations OBSERVED on a local PG16 fixture, SQL regex-extracted from the shipped migration. ⛔ RESIDUAL: the migration has NOT been applied to TEST (`qmnijlgmdhviwzwfyzlc`), so SQL assertion 5 SKIPs in CI and the integration surface the fixture cannot reach — RLS interaction, RPC re-bases against the real `auth`/`strategies` schema, the five post-verifies on real data — is unverified. Merging `supabase/migrations/**` to main AUTO-APPLIES to PROD. Apply to TEST and confirm assertion 5 RUNS before this row goes ✅ |
