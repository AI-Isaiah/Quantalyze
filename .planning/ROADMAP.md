...
- [x] **Phase 140.4: SEAMRIM — close the wizard/client rim the core fix left open** (INSERTED) - Fabricated observations, destructive controls, our-fault-rendered-as-theirs, and the guards that cannot fail (shipped 2026-07-30, PR #652 + CR-01; VERIFICATION **gaps_found** 39/43 — 2 named residuals accepted as tracked tech-debt: **SEAM_MISCONFIGURED→UNKNOWN** on `ConnectKeyStep`/`MultiKeyConnectStep` — the two wizard clients render "Try the last action again" for a fault whose own copy says retrying won't help (⚠️user-facing wrong-retry-copy) + an owed `analytics-client` scrub-test ledger row (doc-hygiene). See TODOS "v1.16 carried-forward residuals")
