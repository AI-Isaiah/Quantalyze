...
**Goal:** Discharge every deferral Phase 164.8.2 produced, and — by founder decision 2026-09-12 — Phase 164.8.6's `[164.8.5-HYGIENE-RESIDUALS]`, which is the same defect class. Nothing else. Four review rounds over that phase's own fixes found ten items that were each, individually, below the bar that would have blocked the ship — and the reason they are one phase rather than ten TODOS lines is that **eight of them are the same defect**: a control narrower than the sentence beside it. 164.8.2 proved three times over that fixing the instance a reviewer named, rather than the class, produces a half-class that the next round finds again.

⭐ **THE FENCE WAS WIDENED ONCE, DELIBERATELY — THIS IS THE RECORD (founder decision, 2026-09-12).** The goal read *"and nothing else"* and that phrasing was meant. It now admits exactly ONE further item, `[164.8.5-HYGIENE-RESIDUALS]`, routed out of Phase 164.8.6 — and admits it ON THE MERITS, because it is this entry's own stated defect class and not a convenient parking space:
...
⚠️ **NOT A PRECEDENT.** A second outside item may be admitted only by the same explicit decision, recorded the same way.
...
6. ~~MYPY-MAINPY-01~~ — ⛔ **MOVED 2026-09-12 to Phase 164.6.1 GATEINFRA** — criterion text travelled with the item; kept as a numbered stub so the original numbering stays stable for the `TODOS.md` and Phase 164.1 cross-references that cite it.
...
8. ~~MT5-GATEWAY-LOGIN-01~~ — ⛔ **MOVED 2026-09-12 to Phase 164.6.1 GATEINFRA** — criterion text travelled with the item; kept as a numbered stub so the original numbering stays stable for the `TODOS.md` and Phase 164.1 cross-references that cite it.
9. ~~CI-DOCSPATH-01~~ — ⛔ **MOVED 2026-09-12 to Phase 164.6.1 GATEINFRA** — criterion text travelled with the item; kept as a numbered stub so the original numbering stays stable for the `TODOS.md` and Phase 164.1 cross-references that cite it.
...
### Phase 164.6.1: GATEINFRA — the mypy strict gate's false coverage claim, the MT5 gateway's unrepeatable hand-login, and a docs-only PR running the whole gate corpus (INSERTED)

**Goal:** Close the three items SPLIT OUT OF PHASE 164.6 by founder decision 2026-09-12, because they are different in kind from lint-and-prose hygiene and each earns its own review posture. (1) **MYPY-MAINPY-01** — `analytics-service/main.py` is 930 lines of RUNNING-SERVICE code (`uvicorn main:app`) sitting OUTSIDE the `mypy --strict` gate, while that gate's own comment at `ci.yml:3209-3216` states the strict floor "now covers ALL running-service code". ⭐ **The FALSE COMMENT is the item, not just the five annotations.** Nothing under `services/`, `routers/` or `models/` imports `main.py`, so `--follow-imports=silent` never reaches it, and `python3 -m mypy --strict main.py` reports 5 errors: `:255` `lifespan(_app: FastAPI)`, `:309` `_crash_handler` (missing `Task[None]`), `:741` ×2 `verify_service_key(request, call_next)`, `:891` `health()`. ⚠️ TWO of the five sit on the service-key middleware Phase 164.1 hardened for PYAPI-06 and one on the `/health` endpoint 164.1's own prober arm polls — the untyped surface is exactly where this milestone has been working. DELIVERABLE: annotate the three functions, add `main.py` to the `ci.yml` mypy invocation, and CORRECT the comment to name the real surface. ⛔ NOT in scope: `analytics-service/tests/` (5,439 strict errors across 182 files) — `TODOS.md:3263`/`:5197` hold that as an OPEN POLICY question, and folding it in would smuggle a milestone-sized decision into a hygiene item. (2) **MT5-GATEWAY-LOGIN-01** — the MT5 terminal's broker session is established ONCE, BY HAND, over VNC, and NOTHING re-establishes it. MEASURED 2026-09-06 while diagnosing issue #747: the prober's credential-less `mt5.initialize()` returned **-6, authorization failed**, while the RPYC bridge was healthy, so the IPC remedies do NOT apply and the fault is the terminal's SAVED SESSION being refused. `railway variables --service mt5-gateway` carries no `MT5_LOGIN`/`MT5_PASSWORD`/`MT5_SERVER`; the session lives solely in the Wine prefix on the `/config` volume. ⭐ **The credentials are not the root cause — the absence of any re-establishment is.** Any rotation, expiry or volume event takes MT5 down SILENTLY, and it stayed silent until 164.1's prober went looking. DELIVERABLE: make login env-driven so a restart self-heals (investor credentials suffice — the gateway only reads equity and positions), and give **-6 its own defect kind and remedy** instead of the catch-all `mt5-terminal-error` whose remedy sends the operator to an error table. ⛔ **NO AGENT ENTERS CREDENTIALS.** Founder decision 2026-09-12: the agent builds the login path, the `-6` defect kind and the restart-self-heals proof, then hands the founder the exact `railway variables --set` commands; the criterion closes only when the founder has set them AND a restart is OBSERVED to self-heal via the prober's mt5 arm. (3) **CI-DOCSPATH-01** — a PR that changes NO code runs the entire gate corpus. MEASURED on PR #750 (four `.planning/` markdown files, zero code): **21 jobs, ~3,001 job-seconds (~50 min)**, excluding `e2e-seeded` and `sql-tests` which were still running. Worse than wasted minutes: `e2e-seeded` and `sql-tests` each take the shared-TEST advisory mutex, so a roadmap typo QUEUES BEHIND and DELAYS real code PRs on a database shared with other people's CI. DELIVERABLE: a path filter so a diff touching only `.planning/**` skips the code-gate jobs. ⛔ **ANTI-VACUITY IS THE WHOLE RISK, and it is why this is a phase rather than a TODO:** a filter that skips jobs has exactly the shape of a gate silently not running, which is this milestone's named defect class. It is NOT shippable on evidence that a docs PR went fast — only on evidence that a CODE PR still runs everything. ⚠️ Beware the GitHub trap that `paths-ignore` on a REQUIRED check reports *pending forever* rather than *passed*, wedging branch protection; a job-level `if:` on a changed-files step is usually safer than workflow-level `paths-ignore`, and whichever is chosen must be demonstrated not to wedge the `frontend` aggregator.

**Success Criteria**:

1. **MYPY-MAINPY-01**: `mypy --strict` covers `main.py` in `ci.yml` and the comment above the invocation names the REAL surface, with no remaining claim of coverage the command does not deliver. ⚠️ Proven by NEUTERING: re-introduce one of the five error shapes and observe the `python` job go RED naming `main.py`. A gate added without that observation is the defect this milestone removes.
2. **MT5-GATEWAY-LOGIN-01**: the gateway re-establishes its broker session WITHOUT a human — proven by RESTARTING the mt5-gateway service and observing the prober's mt5 arm return no defect afterwards, with no VNC session in between. ⛔ Closes only on that OBSERVED restart, never on the code alone (founder decision 2026-09-12, choosing "I build it, you set the variables" over closing on code).
3. **MT5 `-6` is named as itself**: an authorization failure produces its own defect kind with its own remedy, not the catch-all `mt5-terminal-error`. ⚠️ ADJACENT but NOT the same as Phase 164.8.3 PROBERAUTH — check for overlap before planning and fold or defer explicitly rather than shipping the naming twice.
4. **CI-DOCSPATH-01, both halves, and the SECOND is the real criterion**: a docs-only PR no longer runs the code gates, AND a code PR still runs every one of them — proven on TWO REAL PRs, not on a dry run. The `frontend` aggregator is demonstrated not to wedge.
5. **No gate loses coverage silently**: the set of jobs a code PR runs before and after is compared explicitly and any difference is named. ⛔ A shorter job list on a code PR is a REGRESSION, not an optimisation.

**Requirements**: TBD (no v1.20 requirement IDs) + TODOS entries `MYPY-MAINPY-01`, `MT5-GATEWAY-LOGIN-01`, `CI-DOCSPATH-01`

**Depends on:** Phase 164.6 (ordering only — no code dependency; the hygiene half lands first so this phase's CI-path work does not move the gate corpus underneath it)
**Plans:** 0 plans
...
| 164.5.1 CRONREPOINT (the live `match_engine_cron` row repointed; migration-vs-runbook rule settled first) | 0/? | Queued NEXT — measured unblocked 2026-09-12; its six success criteria never mention criterion 3 (created 2026-09-11) | - |
...
| 164.8.3 PROBERAUTH (MT5 `-6` named as "no authorized account", not the catch-all) | 0/? | Queued (created 2026-09-10) | - |
