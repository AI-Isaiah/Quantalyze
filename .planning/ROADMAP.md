...
**Requirements**: TBD (no v1.20 requirement IDs) + TODOS entries OPS-08-F9, OPS-08-F8, OPS-08-TS, OPS-08-F2, PROC-02, PROC-03, H-0001, 161.1-D13, VAC08-COUNT-SPM01 (added 2026-09-05 — VAC-08's ledger ratchet reads its TWO GATING counts, `new_count`/`stale_count` at `scripts/test-ledger-drift-check.sh:372-373`, with the `grep … || true` + `${:-0}` shape the SAME file documents as a false-clean at `:304-318`, where the fix idiom already exists; a grep error on either temp file reads as zero and the gate prints "clean" over unread NEW drift. ⭐ Ship the fix with a test that makes the grep actually fail and asserts MEASURE_FAIL — a fix with no failing test is the defect class 164.3 exists to remove) — read each before planning, do not re-derive
