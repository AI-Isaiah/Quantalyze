...
4. **There is no manifest re-capture script.** `scripts/prod-prober/` contains `cron-manifest.json` and no capture tool, so the re-capture in criterion 2 is hand work unless this phase writes one. The drift arm compares by `command_sha256` and refuses a row without a valid one, so the sha must be regenerated, not edited.
...
1. The migration-vs-runbook question is settled EXPLICITLY and recorded in CONTEXT.md with its reasoning, before any file is written. ⭐ Per the 2026-09-09 narrowing above, the convention half is already answered by two in-tree precedents — the criterion's real content is the SECOND half: state what a rebuild-from-migrations produces, and name the window in which the rebuilt job is broken until the runbook step runs. The losing convention is flagged for cleanup, not silently left to disagree.
