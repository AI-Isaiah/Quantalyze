...
7. ⭐ **`service_role` carries an EXPLICIT `statement_timeout` strictly below the API gateway's 60s ceiling** (45s unless a measurement argues otherwise), set by forward migration — ⛔ NOT by a console `ALTER ROLE`, which leaves no reproducible trace and is the exact class `CRON-DRIFT-01` exists because of. Verified by reading `pg_roles.rolconfig` on PROD AFTER the apply and recording the output, not the statement. ⛔ **This does not make anything faster and must not be described as if it did** — it converts an opaque gateway 504 into a catchable Postgres timeout, so the service can fail loudly and specifically instead of throwing an unhandled `APIError`. It is a legibility fix that makes criterion 8 possible.
...
⚠️ **ADJACENT, CHECK BEFORE PLANNING: Phase 164.8.3 PROBERAUTH** exists to make the prober name MT5 `-6` as "the terminal has no authorized account" instead of the catch-all. The `-6` half of this phase OVERLAPS it. Fold or defer explicitly with a named owner — do NOT ship the naming twice, and do not silently drop either.
...
1. Login is env-driven: the gateway performs `login()` at startup from `MT5_LOGIN`/`MT5_PASSWORD`/`MT5_SERVER`, and the code path is exercised by a test that fails when the login call is removed.
2. ⭐ **The gateway re-establishes its session WITHOUT a human — proven by RESTARTING the mt5-gateway service and OBSERVING the prober's mt5 arm return no defect afterwards, with NO VNC session in between.** ⛔ This is the phase's load-bearing criterion and it closes on that observation alone.
3. **-6 is named as itself**: an authorization failure produces its own defect kind with its own remedy, not the catch-all `mt5-terminal-error`. Proven by forcing the classifier over a `-6` input and asserting the named kind.
4. ⛔ NOT in scope: entering any credential, and changing what the gateway READS. This phase removes the need to repeat a hand login; it does not widen access.
...
- [ ] TBD (run /gsd-plan-phase 164.6.2 to break down)
