...
| 3 | N2: `SELECT … FOR UPDATE` in `revoke_strategy_share`, **and** STEP 6 arm (i-b) rewritten so removing the racy predicate no longer fails the apply | **164-06** | ⭐ **DOES NOT REPRODUCE (2026-08-27)** — 3 interleavings, 2 concurrent sessions each, all converge. Both RPCs are single statements; there is no read-then-write window for a `FOR UPDATE` to protect. ⛔ And `revoked_at IS NULL` is the convergence CONTRACT, not a racy predicate — rewriting arm (i-b) so the fix could land would have removed the guard and CREATED the counter-inflation bug. Recommend closing as not-a-defect and dropping N2 from 164-06, leaving it N1-only. **Founder call** — the corpus records N2 as `[M]`. See `EXECUTION-EVIDENCE.md` §6 |
