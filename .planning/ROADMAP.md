...
9. ⭐ **`[164.6.2-RAISE-LAST-SHAPE-ONLY]` STAYS IN THIS PHASE AS A POST-SHIP PLAN OF ITS OWN — ⛔ explicitly NOT inside wave 5, and ⛔ NOT in Phase 164.6 GATE-HYGIENE (founder decision 2, 2026-09-14).** `Mt5Client.login`'s falsy arm goes through the shared `_raise_last`, which scrubs by SHAPE only, so a bare broker-server literal echoed back by the terminal survives into the message. **PRE-EXISTING on every shipped call site and NOT a regression**, and it is now MEASURED rather than asserted — `test_CREDENTIAL_REDACTION_the_falsy_arm_asymmetry_is_measured_not_assumed` pins the contrast in both directions and will red loudly when a future phase closes it. ⛔ **Why not 164.6:** kind mismatch — it edits `Mt5Client.login()` on the LIVE job-processing path, while 164.6's remaining scope is lint rules, tests and prose corrections; bundling a live-path code edit there would let one red gate block three unrelated items. ⛔ **Why not wave 5:** wave 5's instruments must stay READ-ONLY — a wave that both mutates a credential path and measures production would destroy the observation criterion 2 exists to make. ⚠️ If the founder prefers a dedicated phase instead, it is one `/gsd-phase --insert` and nothing here blocks that.
...
11. ⛔ **THE PHASE DOES NOT CLOSE YET — founder decision 2026-09-15 at wave 5's checkpoint, option `book-keepalive`.** Criterion 2 is closed, but two items remain open in this phase and `phase_closes: false`: **plan 06**, which takes question THREE's R2 after the next REAL per-account login (evidenced in the analytics log) and only THEN decides whether it edits the four stale-sentence copies, carrying plan 04 task 2's gates verbatim but licensed by the WAVE-5 record's verdict line rather than OUTCOME 0's; and `[164.6.2-RAISE-LAST-SHAPE-ONLY]` as its own post-ship plan per criterion 9, unchanged by this wave. ⭐ **The keepalive is BOOKED as a NAMED PHASE OF ITS OWN (Phase 164.6.4 MT5KEEPALIVE) and ⛔ ITS INTERVAL IS DELIBERATELY NOT CHOSEN HERE.** Wave 5 produced exactly ONE observation of each quantity, and an interval set from n=1 is still the guess D-06 exists to prevent; the new phase owns designing it against more than one measurement. ⚠️ `[164.6.2-PLAN04-GATE-INTENT-DRIFT]` stays routed to Phase 164.8.4 GATERESIDUE — no new deferral — with wave 5's exact exit-code semantics recorded in `164.6.2-05-SUMMARY.md` instead.
...
1. ⭐ The interval is chosen against **MORE THAN ONE** measured session lifetime, and the measurements are recorded with their stamps. ⛔ Never a number carried over from wave 5's single observation.
...
5. ⛔ NOT in scope: re-litigating Phase 164.6.2 criterion 1's worker-side shape (a founder decision taken on a measurement), and changing what the gateway READS.
...
- [ ] TBD (run /gsd-plan-phase 164.6.4 to break down)
