...
- [ ] 167-06-PLAN.md — wave 4 · **gap closure** (re-verification gap 1): a MANAGER's untrusted key shows the existing amber pill + authored helper on its own card on `/strategies/[id]/edit`, with rules R1–R6 so the local sync panel never contradicts it (D-18); goal truth 1 closed by D-19
