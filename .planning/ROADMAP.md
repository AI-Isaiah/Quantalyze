...
- [ ] 164.5-06-PLAN.md — crit 3 / DRIFT-04: the DROP migration and its pre-flight. ⛔ **BUILT AND REVIEWED, NOT APPLIED, AND NOT IN THIS PR.** One-way production DDL on a credential-bearing SECURITY DEFINER function; three reviewers ran (rls-policy-auditor SAFE, migration-reviewer DO NOT APPLY YET, silent-failure-hunter NOT TRUSTWORTHY — all findings fixed). The PROD pre-flight measurements were taken read-only and all four assertions pass, but the GATE SCRIPT ITSELF has never run against PROD. Awaiting the founder.
...
1. The migration-vs-runbook conflict above is settled EXPLICITLY and recorded in CONTEXT.md with its reasoning, before any file is written. The losing convention is flagged for cleanup, not silently left to disagree.
