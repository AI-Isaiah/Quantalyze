...
- [ ] 167.2-01-PLAN.md — one compute-state derivation (chain-kind selection, done_pending_children in flight), the sync-progress route on it, the locked S4-S9 copy module and the one strategy-shape predicate (KCS-07/08/19/20/21)
...
- [ ] 167.2-03-PLAN.md — a terminal is accepted only with this attempt's server-written evidence (KCS-02)
- [ ] 167.2-07-PLAN.md — the owner pending factsheet states the real compute state with a shape-true remedy; the public placeholder is one neutral sentence; the owner share panel says what a recipient sees (KCS-09/10/12/21)
