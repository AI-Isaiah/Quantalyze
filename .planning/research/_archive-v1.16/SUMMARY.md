...
| 5 | JOB: "Unified-backbone CSV-finalize breaks if flag on — service-role client has no `auth.uid()` → 42501" | **Likely stale.** `process_key.py` (~lines 792-820) already forwards `X-User-Access-Token` and calls `finalize_csv_strategy` with a user-scoped client, with an inline comment describing exactly this bug and fix. `docs/runbooks/compute-queue.md` says `PROCESS_KEY_UNIFIED_BACKBONE` is **retired** (reads nowhere). | Require a **reproduction attempt against current `main` BEFORE any fix is scoped**. "Could not reproduce" is a valid outcome. The genuinely-open adjacent gap is different: csv-finalize's multi-step sequence (RPC → RPC → `after()` enqueue) is non-transactional. |
