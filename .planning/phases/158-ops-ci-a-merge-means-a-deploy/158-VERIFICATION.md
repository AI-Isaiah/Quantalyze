...
human_verification:
  - test: "Phase-PR live CI run (one run covers four recorded backstops): (a) plant a temporarily failing supabase/tests assertion → frontend concludes FAILURE with sql-tests=failure in the loop output, revert → green (both polarities); (b) read the e2e + e2e-seeded Playwright per-spec output — each of the five newly wired specs (api-key-flow, sync-analytics-flow, full-flow, csv-upload-flow, my-strategies) reports >=1 executed non-skipped case; (c) the three 'Acquire shared-test-db mutex' steps run in situ (background psql survives step boundaries; release step fires); (d) the unseeded job's NEXT_PUBLIC_ALLOWED_ORIGINS line works in production mode (no 'Origin not allowed' reds); (e) test_defer_compute_job_token_fence / _null_token_backcompat execute for the first time (they skip locally without TEST env)"
    ...
    result: "PASSED 2026-08-21 — see 158-UAT.md item 1. (a) both polarities: PR #697 run 32424762495 all-green; drill PR #698 run 32426772489 sql-tests FAILED on planted assertion and frontend concluded FAILURE with sql-tests=failure in the loop output. (b) CI dot reporter cannot attribute per-spec, so each wired spec was executed individually at HEAD under its batch env: 3/4/4/2/1 executed cases, zero failures. (c) all three acquire steps ran; the e2e-seeded 75s wait was later shown to be the holder-death fault signature, not orderly contention — see the MUTEX-01 note in Known-open and 158-UAT.md item 1(d). (d) unseeded job green incl. the repaired specs (403-without-origin arm exercised locally under the exact CI env). (e) fencing tests executed in the python job (~30 cases)."
...
**Verified:** in-tree 2026-08-20T23:59Z at HEAD `4ecd2c01`; backstops discharged 2026-08-21T05:05Z on `feat/v1.20-phase-158` (phase-start base: `35c74149`; merge-base at the verified HEAD was `ca3f0c5c` after the pre-ship base merge)
...
**[158-MUTEX-01] Holder dies at ~120s — mutual exclusion holds only the first ~2 minutes of each job's DB span (found by the ship adversarial review, 2026-08-21, AFTER the closure measurements).** The TEST project's Postgres sets server-wide `statement_timeout=120000` (measured via `pg_settings`, source "configuration file"; the `postgres` role carries no per-role override). The holder's `SELECT pg_sleep(6000)` is a single statement, killed at 120s → psql exits → session closes → advisory lock released while the job's DB work continues. ci.yml's own release-step detection fired on EVERY long-job evidence run (`##[error] mutex holder pid … died … ran UNSERIALIZED`: python + e2e-seeded on 32424762495, python + e2e-seeded on 32426772489, e2e-seeded on 32447308698); the probe never trips it (45s holds < 120s). A contended acquire waiting >120s is at risk through the same mechanism (the retry loop masks single kills). The WR-01 invariant (sleep > job TTL) did not consider server-side statement kill. OPS-01's headline outcome — no eviction, a cancelled conclusion is loud, a merge deploys — is unaffected and live-proven; what is degraded is the serialization guarantee's DURATION. Fix is one line (`SET statement_timeout = 0` in the holder session, and on the acquire statement) — tracked P0 as TODOS.md [158-MUTEX-01], to ship as its own reviewed PR immediately after this closure lands. Credential rotation (human action, TODOS + AR-158-3); SEC-02 `.planning/` credential sweep (Phase 163 scope); WR-06 promotion decision; auth.users leak convention; csv-upload-flow's 2 cases skipped until `ANALYTICS_SERVICE_URL` exists in CI; 15 remaining orphan specs (one-missing-convention root cause, dispositioned); DB-types no-gate decision (that IS the OPS-03 deliverable).

### Gaps Summary

No gaps beyond the tracked [158-MUTEX-01] holder-lifetime defect above (found post-closure by the ship adversarial review; detection layer worked, duration guarantee degraded, P0 fix queued). Every in-tree must-have is verified with evidence at HEAD, including live re-observation of the drain interlocks, the probe conclusions on GitHub, and two named test suites run by the verifier under CI-parity Node 22. The four plan-recorded backstop truths — abstained on 2026-08-20 per the honest-verifier contract — were discharged 2026-08-21 by the phase PR's live CI run and the post-merge dispatches, so the phase is honestly `passed` on measurement, not assertion.
