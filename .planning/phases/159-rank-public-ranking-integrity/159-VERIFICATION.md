...
behavior_unverified_discharged: 1
closed: 2026-09-12T20:00:00Z  # the single open item discharged by measurement at 733a55f5; see `discharged:` below
overrides_applied: 0
behavior_unverified_items:
  - truth: "The CAS behaves against a REAL PostgREST/Postgres the way the mock models it (zero-row UPDATE returns data:[] with error:null)"
    ...
    blocked: "SEQUENCED, not stalled — blocked on Phase 164.3 (2026-08-28). This item and its sibling measure the same thing and need the same missing capability: REQUEST-LEVEL control, i.e. two csv-finalize POSTs carrying the SAME wizard_session_id against a real Postgres. A browser cannot produce it — re-entering the wizard after a completed submit mints a FRESH session (observed directly on production 2026-08-28 while discharging the 409-remedy item), which is correct behaviour and is exactly why the conflict arc is unreachable from the UI. It must NOT be driven against Supabase TEST either: TEST is shared, and a deliberate write race on it is the shape that reddens other peoples CI. ⛔ RE-POINTED 2026-08-29 (Phase 164.3 verification gap G2): this item is now blocked on PHASE 164.5, not 164.3. 164.3 landed and DEFERRED VAC-07 by founder decision — see TODOS.md [VAC-07-DEFER] and .planning/phases/164.3-.../164.3-07-DEFERRED.md. The previous recipe (that it would be cheap once 164.3 landed) was FALSIFIED on two counts by plan 04. (a) WRONG LANE: this spec is app-level — csv-finalize and withAuth speak PostgREST/GoTrue through supabase-js and the seed helper needs GoTrue admin — so it needs the SUPABASE LOCAL STACK (D-15), not the bare initdb pg-lane 164.3 shipped for SQL gates. (b) THE LANE CANNOT BOOT: the migration chain does not replay from empty (262 files, 69 fail, ≥6 independent causes, one refusing BY DESIGN on an unidentifiable database — scripts/local-stack/REPLAY-SPIKE.md), so the lane loads a committed schema baseline instead, and scripts/local-stack/run.sh still points at a gitignored path that does not exist and dies FATAL. CORRECTED CLOSING RECIPE, and it is no longer cheap: FIRST 164.5 must repoint run.sh at supabase/schema/baseline.sql, drop .gitignore:138 and add the baseline staleness gate; THEN drive both items on that baseline-loaded local stack as ONE spec over e2e/helpers/seed-test-project.ts — two concurrent finalize POSTs on one never-classified session; assert exactly one 2xx applied receipt, one honest raced refusal, and that category_id holds the winner. That single spec discharges this item AND the PostgREST zero-row-UPDATE truth currently carried by a mock whose fidelity rests on precedent plus docs."
    discharged: "DISCHARGED 2026-09-12 — the closing recipe this entry specifies was BUILT and is GREEN. (1) The 164.5 repoint landed: `scripts/local-stack/run.sh:57` reads BASELINE_FILE=${REPO_ROOT}/supabase/schema/baseline.sql, and that file is tracked and non-empty (746,969 bytes at 733a55f5) — the FATAL gitignored-path death is gone. (2) The spec exists as ONE spec, exactly as the recipe asked: `src/__tests__/csv-finalize-concurrent-never-classified.test.ts`, collected by `vitest.local-stack.config.ts` as two cases — a SETUP case pinning that the two clients hold DIFFERENT access tokens (a single client cannot open the race) and `VAC-07: exactly ONE applied receipt, one NAMED refusal, and category_id holds the winner`. (3) ⭐ It cannot pass vacuously, which is the whole point: the file carries NO skip gate by deliberate design (`This spec FAILS rather than skips on an absent lane — a silently skipped live-DB` … at :156), so a green lane means the race RAN. (4) It runs on a BLOCKING CI lane: `frontend-local-stack` in `.github/workflows/ci.yml:495`, whose job also MEASURE_FAILs on an empty corpus. SHA-BOUND: `frontend-local-stack: success` in main CI run 34712535912 at head 733a55f5. The PostgREST zero-row-UPDATE truth this entry says rests on a mock plus precedent is now measured against a real Postgres. Neither TEST nor PROD was written to — the lane is a disposable local stack, honouring the do-NOT-race-shared-TEST constraint stated above."
human_verification:
  ...
  - test: "End-to-end 409 remedy (159-07 D4): after a classification-conflict 409, re-classify and resubmit in the real wizard"
    ...
    result: "PARTIALLY DISCHARGED 2026-08-28 — the composed flow WAS driven against the live production route, which is what this item asked for; the 409 refusal arm itself was NOT reachable. Driven as sm@quantalyze.test on quantalyze.xyz with an adapted daily_returns CSV: POST /api/strategies/csv-validate 200 (515 rows, 2023-10-01..2025-02-26), POST /api/strategies/csv-finalize 200, strategy 6f1a891f-78e5-4e88-9a74-96286908ceaa created with wizard_session_id e2903af1-09e5-4a07-995d-fcbaa8322c0c. ⛔ The 409 could NOT be provoked through the UI: re-entering the wizard after a completed submit lands on a FRESH session at step 01/05 (\"Not saved yet\"), never resubmitting the consumed session. That is the expected-behaviour half of this very item (\"a fresh wizard_session_id is minted\") observed directly — but it means the conflict-then-remedy ARC cannot be produced by a browser at all. Provoking a real 409 needs a direct re-POST of csv-finalize carrying the SAME wizard_session_id, i.e. request-level control, which places this item in the same bucket as the concurrent-CAS item above. ⭐ Both are cheap once Phase 164.3 ships its disposable-Postgres lane; recommend closing both there as one Playwright spec against e2e/helpers/seed-test-project.ts rather than by hand. Test artifact archived (status=archived) after the run; sm@ published set untouched at 2."
