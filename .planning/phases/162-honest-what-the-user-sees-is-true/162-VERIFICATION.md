...
human_verification:
  ...
  - test: "Real-browser pass over the three new UI surfaces: (a) factsheet v2 masthead — FreshnessChip + 'Track record through {date}' line on a strategy with an old series end (chip should read 'Track record · old', never 'Computed · fresh'); (b) my-strategies 'Finish setup →' on an orphaned key — wizard opens with that key's exchange+label summarized, focus on 'Continue with this key', continues into a draft without a KEY_ORPHANED refusal; (c) allocations drawer-add — em-dash cells while loading, real CAGR/Sharpe after settle"
    ...
    recheck_2026_09_18: "⛔ THE STATED BLOCKER IS VOID — RE-TESTED, NOT INHERITED. The 2026-08-28 reading ended `blocked on a working display, not on effort — do not re-attempt it from this session`. That was true of THAT session. Re-measured in a real browser on 2026-09-18: viewport `1633x865`, pages paint, screenshots capture. So (c) is REACHABLE now, and the instruction not to re-attempt should not be carried forward by a future reader. (b) is UNCHANGED and still needs the precondition to exist for the tester. ⭐ (c) MEASURED, AND IT IS THE CRITERION'S OWN OBSERVABLE. The surface is the ScenarioComposer drawer-add (`/allocations?tab=scenario` -> `+ Strategy` -> `Add`), and the truth under test is 162-04's: `In-flight fetch renders em-dash cells with NO absence note — the note is a settled claim and must not flash during load`. A transient cannot be read from served HTML, so it was sampled: 50 polls at 40ms across the add of `Strategy #fc1b4014`. Result: the string `No computed metrics for this strategy` appeared in **0 of 50 samples**, while the em-dash count rose 42 -> 43 — one new absent cell, no note beside it. That is the predicted behaviour exactly. ⚠️ HONEST LIMIT, AND IT IS WHY THIS IS NOT MARKED PASS: the observation is UNCALIBRATED. No leg reached the settled-BOTH-null state during the session, so the note was never shown to be capable of rendering against this account's data. Per this repo's own anti-vacuity rule, `the thing that must not flash never flashed` is weak evidence until `the thing can flash` is also shown — a note deleted from the component would produce an identical reading. To finish (c): reach a settled-both-null leg (a strategy with no computed metrics INSIDE the coverage window), confirm the note DOES render there, then re-run the poll. ⚠️ A third leg state was found and is worth recording: `Strategy #8581f739` added as `OUTSIDE WINDOW — ends May 2026 — outside window`, which is a different honest surface than either em-dash case and is not covered by the C-4 state matrix. ⛔ HOUSEKEEPING: two draft legs were added to drive this and the draft was restored to its pre-session state (`No changes yet`, `1 strategy — not a blend`) via Discard draft; nothing was committed, nothing was saved, and the only network calls the add made were `GET /api/strategies/{id}/returns`."
    founder_ruling_2026_09_18: "CLOSED. Founder: `if you can't reach it, then this is obviously not a matter anymore. remember, only user-facing and data-integrity issues`. Both remaining legs are UNREACHABLE rather than unverified — (b) needs an orphaned key on an account the tester can sign into (fleet-wide the state exists on 24 keys, on none of ours), and (c) needs a leg that settles BOTH-null inside the coverage window, which this account's data does not contain. ⭐ THE ANTI-VACUITY OBJECTION IS WITHDRAWN BY SCOPE, NOT WAIVED: the repo's calibration rule is scoped to gates pinning DATA-INTEGRITY or USER-FACING behaviour, and `the settled-absent note was never shown able to render` is a calibration gap in a VERIFIER'S OWN OBSERVATION, not an ungated product behaviour — the note's five states are already pinned in `ScenarioComposer.added-metrics.test.tsx`. What WAS observed in a real browser stands on its own: 0 note appearances in 50 samples at 40ms across a live drawer-add, with a new em-dash cell appearing. Item 3 is therefore fully discharged: (a) PASSED on exact predicted wording, (b) and (c) closed as unreachable."
