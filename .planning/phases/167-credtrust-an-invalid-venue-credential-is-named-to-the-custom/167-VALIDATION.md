...
| D-16 | A holding whose source key carries the new status is NOT rendered as trusted live data; the 7 former `revoked` equalities are callers of ONE untrusted-status predicate | unit | `npx vitest run "src/app/(dashboard)/allocations/" --reporter=dot` | ✅ — **owned by `167-04` task 3** (added 2026-09-22 by founder decision D-16, after plan 03's D-13 review round found it on two surfaces independently) |
...
| gap 1 / D-18 | A manager's untrusted key renders the existing pill and helper on its key card, healthy keys render nothing new, and the card and the local sync panel never contradict each other (R1–R6) | unit | `npx vitest run src/components/strategy/ApiKeyManager.test.tsx --reporter=dot` | ✅ — **owned by `167-06` tasks 1–2** (added 2026-09-22, gap closure) |
