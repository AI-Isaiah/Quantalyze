...
> Regenerated by the orchestrator from `ui-consideration-probe.cjs` after checker approval, so the
> rows are keyed to the probe's own (element, category) pairs rather than to prose headings.
> Probe output: **10 applicable** across S1 and S2. Substance is the UI researcher's; the
> attribution and the two added rows are the probe's. ⛔ This section covers shape-rooted STATE
> coverage and REFERENCES the copy rows — it never restates copy (de-dup rule).

Resolved: **8 explicit, 1 backstop, 1 unresolved.**

| Element | Category | Status | Resolution / Reason |
|---|---|---|---|
| S1 | overflow | ✅ explicit | Length budget ≤60 chars declared in `## Typography` against the measured 41/40/45-char house strings; the specified helper string is 58. The helper sits in a right-aligned flex column and wraps rather than clipping. |
| S1 | long-text | 🧪 backstop | The helper is a FIXED authored string with no interpolation, so "unusually long text" cannot arise from data — only from viewport. At 320px or 200% zoom it wraps; wrapping is acceptable for a caption. ⚠️ Not verified at those widths this session. **Held-out visual check owed.** |
| S2 | empty | ✅ explicit | Structurally unreachable: the envelope renders only on a verdict-or-failure, never on absent data. Declared in the 9-state *empty* cell with that reason. |
| S2 | loading | ✅ explicit | The existing submit-busy state owns this; the credential sentence is silent while a validate is in flight. Declared in the 9-state *loading* cell. No new copy. |
| S2 | error | ✅ explicit | This IS the error state. Copy specified verbatim in `## Copywriting Contract` §2; tone in `## Color`, live region in `## Accessibility`. |
| S2 | populated | ✅ explicit | The happy path does not render an envelope at all — a successful validate advances the step. Declared in the 9-state *success* cell. |
| S2 | partial | ✅ explicit | Structurally unreachable: a validate returns a verdict or a failure, never a partial. Declared with that reason, not asserted. |
| S2 | overflow | ⚠ **unresolved** | The `cause` paragraph is longer than most entries in the table. `ErrorEnvelope`'s `px-4 py-3` shell reflows and the wizard has been CSS-first responsive since Phase 46, so no horizontal overflow is expected — **but no measurement was taken.** ⛔ The planner treats this as an ASSUMPTION, not a fact. |
| S2 | zero-one-many | ✅ explicit | `fixRequires` is index-aligned to `fix`, so bullet count varies by venue and surface: MT5 renders 4, every other venue 3, an unknown venue 3, and the `preselect` render one fewer. All four counts read as a complete list — ⭐ no bullet depends on a sibling being present. |
| S2 | long-text | ✅ explicit | Every string in the entry is authored and static — ⛔ the copy interpolates NO venue-supplied text, so a hostile or verbose upstream message cannot reach this surface. That is the same property that makes the entry venue-agnostic (`## Copywriting Contract` §3). |

⚠️ **Why S1 draws only two categories.** The probe classifies a status pill plus a caption as a
text-bearing element, not a data container, so `empty` / `populated` / `partial` / `zero-one-many`
are not applicable to it. S1's behaviour in those states is still declared — in the 9-State Matrix,
which covers all 9 states for both surfaces — this table covers the shape-rooted axis only.
...
- [x] Dimension 4 Typography: PASS — FLAG: 3 font weights declared (400/500/600). Both are INHERITED locks (500 is the existing pill's `font-medium`, 600 is DESIGN.md §Error Envelope / REQ DESIGN-02) and both new strings land in the 400 caption slot. The spec introduces no weight; recorded so the count is on the record.
- [x] Dimension 5 Spacing: PASS
- [x] Dimension 6 Registry Safety: PASS
- [x] Dimension 7 Inventory Provenance: PASS — count and version independently re-derived and matched.

**Approval:** approved 2026-09-22 (gsd-ui-checker, 7 PASS / 0 BLOCK / 3 non-blocking FLAGs)

### Checker findings carried forward for the PLANNER

1. ⛔⛔ **Arm C's "narrow the `error` branch" touches a SHARED branch, and this spec did not say so.**
   MEASURED at HEAD in `AllocatorSyncStatus.tsx`: the helper-resolution condition is
   `normalized === "error" || normalized === "complete_with_warnings"` — **ONE branch serving BOTH
   states**, both returning `syncError ?? ""`. The 9-State Matrix's *partial* cell asserts
   `complete_with_warnings` is UNCHANGED, which is the right intent, but arm C's narrowing edits the
   exact expression that also serves it. ⭐ **The plan must SPLIT the condition, not filter inside
   it**, or it silently changes a state nobody asked it to touch.
2. ⭐ **The inverted D-11 cost claim is CONFIRMED against shipped code.** The chain reaches
   `helperText = REVOKED_HELPER` on the `revoked` branch BEFORE the `error` branch, so arm A
   genuinely requires REWORDING a locked constant whose text is venue-wrong for MT5 — not extending
   the chain. Arms B and C are pure extensions. ⛔ Do not carry the usual "arm A is cheapest" framing
   into the costing.
3. ⚠️ **Arm B's silent side effect re-confirmed:** `PILL_STYLES` normalises an unknown `rawKey` to
   `"idle"`, a NEUTRAL pill — so a partially-rolled-out value renders a BROKEN key as HEALTHY. The
   style map and the migration land in the SAME commit.
4. ⚠️ **Contrast decimals re-derived independently** (WCAG 2.x sRGB, 0.04045 knee, alpha
   composited). Direction holds on all four; two figures here are marginally optimistic — the
   helper measures **4.76:1** (not 4.85), the amber trio **4.51:1** (not 4.55), `#4A5568` on the
   envelope shell **6.96:1** (stated as ≥7). All AA verdicts are unaffected. Noted so a later reader
   re-deriving them is not surprised.
5. ⚠️ **Open Question 3 is REAL and reproduced exactly:** the shipped `rate_limited` and
   `complete_with_warnings` pills (`bg-warning/10 text-warning`, 12px) land at **4.39:1** over white
   and **4.16:1** over the page background — below DESIGN.md's own 4.5:1 minimum. ⛔ Out of THIS
   phase's scope (fixing it means restyling a locked map) and ⛔ must not be quietly dropped.
