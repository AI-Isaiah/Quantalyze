...
| 150-05-01 | 05 | 2 | OWN-03 (D-13, D-14, SC 4) | T-150-21/22/23/24/26 | upsert-shaped with literal onConflict assertion; pre-checks 404/409; no current_weight; $1e9 MAX_TICKET_SIZE_USD cap (review round 3 W-3) enforced before token burn | route unit | `npx vitest run "src/app/api/portfolio-strategies/allocation/route.test.ts" --no-file-parallelism` | ❌ W0 | ⬜ pending |
...
| 150-07-03 | 07 | 3 | OWN-03 (review round 1 W-6, B-3) | — | server-fetch wiring, no client reads; getMyStrategies-count empty-arm discriminator; B-3 slack removed | tsc + tree | `npx tsc --noEmit && npx vitest run "src/app/(dashboard)/allocations" --no-file-parallelism` | ✅ | ⬜ pending |
| 150-07-03b | 07 | 3 | OWN-03 (rev-3 blocker: null-portfolio remedy) | — | props.portfolio null → `Allocate…` stays clickable; remedy state (`No portfolio yet.` + /portfolios link) renders instead of the amount form; ZERO fetch | unit (RTL) | `npx vitest run "src/app/(dashboard)/allocations/components/HoldingsTable.strategy-rows.test.tsx" --no-file-parallelism` | ✅ extend | ⬜ pending |
...
| money (legacy weight source dead — review round 2 W-1) | `strategies-row-adapter.ts`: re-add `weight: ps.current_weight` to the position-row mapping | adapter positioned-row-with-current_weight → `weight: null` case (strategies-row-adapter.test.ts) | ⬜ pending | |
| SC-2 (null-portfolio remedy — rev-3 blocker) | Plan 07 dialog host: remove the null-portfolio branch (mount AllocateDialog unconditionally / assume `props.portfolio!.id`) | HoldingsTable.strategy-rows.test.tsx portfolio-null case: remedy copy rendered (`No portfolio yet.` + `/portfolios` href), ZERO fetch | ⬜ pending | |
...
- [ ] Sampling continuity: no 3 consecutive tasks without automated verify (map above: every task has one; the Plan-01 checkpoint now carries the post-apply psql pgTAP run as its automated half — review round 1 W-2)
