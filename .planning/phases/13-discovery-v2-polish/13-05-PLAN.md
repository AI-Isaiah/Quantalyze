...
must_haves:
  ...
  artifacts:
    - path: "supabase/migrations/091_seed_is_example_backfill.sql"
  ...
  key_links:
    - from: "supabase/migrations/091_seed_is_example_backfill.sql"
...
@supabase/migrations/090_claim_dedupe_partition_keys.sql
...
From `supabase/migrations/090_claim_dedupe_partition_keys.sql` — confirms the highest migration number on disk is 090. Phase 13's only DML migration takes the next free number = **091** (089 is `089_claim_failed_retry.sql` from PR #82; 090 is `090_claim_dedupe_partition_keys.sql` from PR #83).
...
<task type="auto">
  ...
  <files>supabase/migrations/091_seed_is_example_backfill.sql</files>
  <read_first>
    ...
    - supabase/migrations/090_claim_dedupe_partition_keys.sql (highest existing migration number; main now has 090_claim_dedupe_partition_keys — confirms 091 is next free for Phase 13)
  ...
  <action>
Create `supabase/migrations/091_seed_is_example_backfill.sql` with the following content (use the Write tool — never heredoc cat):
...
After writing, do NOT push yet — Task 2 handles the push under the `autonomous: false` checkpoint pattern.
  ...
  <verify>
    <automated>test -f supabase/migrations/091_seed_is_example_backfill.sql && grep -c "cccccccc-0001-4000-8000-00000000000" supabase/migrations/091_seed_is_example_backfill.sql</automated>
  </verify>
  <acceptance_criteria>
    - File `supabase/migrations/091_seed_is_example_backfill.sql` exists
    - Filename uses the next-free number — `ls supabase/migrations/ | grep -c "^090_"` == 1 (only the new file at 090)
    - `grep -c "UPDATE.*strategies\\|UPDATE public.strategies" supabase/migrations/091_seed_is_example_backfill.sql` >= 1
    - `grep -c "SET is_example = true" supabase/migrations/091_seed_is_example_backfill.sql` >= 1
    - `grep -c "cccccccc-0001-4000-8000-00000000000" supabase/migrations/091_seed_is_example_backfill.sql` == 16 (8 UUIDs × 2 occurrences — once in UPDATE, once in DO probe)
    - Inverted: `! grep -E "ALTER TABLE|CREATE TABLE|DROP TABLE|ALTER COLUMN|ADD COLUMN" supabase/migrations/091_seed_is_example_backfill.sql` (data-only — no DDL)
    - Inverted: `! grep "ON CONFLICT" supabase/migrations/091_seed_is_example_backfill.sql` (UPDATE not UPSERT — rows already exist by definition)
    - `grep -c "DO \\$\\$" supabase/migrations/091_seed_is_example_backfill.sql` >= 1 (post-update probe)
    - `grep -c "RAISE NOTICE" supabase/migrations/091_seed_is_example_backfill.sql` >= 1
  </acceptance_criteria>
...
<task type="checkpoint:human-action" gate="blocking">
  ...
  <files>supabase/migrations/091_seed_is_example_backfill.sql (read-only — applied to remote DB; no source edits in this task), .planning/phases/13-discovery-v2-polish/TODOS.md (append "DISCO-05 backfill" section)</files>
  <read_first>
    - supabase/migrations/091_seed_is_example_backfill.sql (the migration to push)
    - .planning/phases/13-discovery-v2-polish/TODOS.md (Migration numbering section — confirms 091 is next free for Phase 13; 089 is 089_claim_failed_retry from PR #82, 090 is 090_claim_dedupe_partition_keys from PR #83)
...
2. Verify the migration file exists: `ls supabase/migrations/091_seed_is_example_backfill.sql`
...
- Failure mode C: the seed UUIDs aren't in production at all → escalate; the Task 2 audit will have shown count=0
  ...
  <acceptance_criteria>
    ...
    - `grep -c "seedTestAllocator\\|cleanupTestAllocator" e2e/discovery-hide-examples-default.spec.ts` >= 2 (uses existing test-helper infra)
...
- `grep -c "is_example = true" supabase/migrations/091_seed_is_example_backfill.sql` >= 1
- `grep -c "cccccccc-0001-4000-8000-00000000000" supabase/migrations/091_seed_is_example_backfill.sql` == 16 (8 UUIDs × 2 occurrences)
- `grep -c "test.skip" e2e/discovery-hide-examples-default.spec.ts` >= 1

**Negative checks:**
- `! grep -E "ALTER TABLE|CREATE TABLE|DROP TABLE" supabase/migrations/091_seed_is_example_backfill.sql` (data-only)
...
2. **CONTEXT.md DISCO-05 locked decisions:**
   ...
   - Idempotent
3. **Migration numbering:** 091 is the next-free number (089 is `089_claim_failed_retry.sql` from PR #82 and 090 is `090_claim_dedupe_partition_keys.sql` from PR #83 — the highest existing per TODOS.md and `ls supabase/migrations/`).
