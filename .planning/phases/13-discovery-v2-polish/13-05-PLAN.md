...
depends_on:
  - 13-02
files_modified:
  - supabase/migrations/089_seed_is_example_backfill.sql
...
must_haves:
  truths:
    - "All 8 seed strategy UUIDs (cccccccc-0001-4000-8000-000000000001 through ..-000000000008) have is_example=true after migration 089 applies"
    - "Migration 089 is data-only DML (UPDATE strategies SET is_example=true WHERE id IN (...)) — no DDL, no ALTER TABLE"
    - "Migration 089 is idempotent — running twice is a no-op (set-to-true is the same as set-to-true)"
  ...
  artifacts:
    - path: "supabase/migrations/089_seed_is_example_backfill.sql"
  ...
  key_links:
    - from: "supabase/migrations/089_seed_is_example_backfill.sql"
...
@supabase/migrations/088_cutover_strategy_metrics_keys.sql
...
From `supabase/migrations/088_cutover_strategy_metrics_keys.sql` — confirms the highest migration number on disk is 088. Phase 13's only DML migration takes the next free number = **089**.
...
<task type="auto">
  <name>Task 1: Author migration 089 — idempotent is_example=true backfill on seed UUIDs</name>
  <files>supabase/migrations/089_seed_is_example_backfill.sql</files>
  <read_first>
    ...
    - supabase/migrations/088_cutover_strategy_metrics_keys.sql (highest existing migration number — confirms 089 is next free)
  ...
  <action>
Create `supabase/migrations/089_seed_is_example_backfill.sql` with the following content (use the Write tool — never heredoc cat):

```sql
-- 089_seed_is_example_backfill.sql
--
...
BEGIN
  ...
  RAISE NOTICE '[089_seed_is_example_backfill] flagged % seed rows with is_example=true (expected 8 if all seeds present in this DB; lower in fresh test DBs where some seeds may not exist)', flagged_count;
END $$;
```
...
After writing, do NOT push yet — Task 2 handles the push under the `autonomous: false` checkpoint pattern.
  ...
  <verify>
    <automated>test -f supabase/migrations/089_seed_is_example_backfill.sql && grep -c "cccccccc-0001-4000-8000-00000000000" supabase/migrations/089_seed_is_example_backfill.sql</automated>
  </verify>
  <acceptance_criteria>
    - File `supabase/migrations/089_seed_is_example_backfill.sql` exists
    - Filename uses the next-free number — `ls supabase/migrations/ | grep -c "^089_"` == 1 (only the new file at 089)
    - `grep -c "UPDATE.*strategies\\|UPDATE public.strategies" supabase/migrations/089_seed_is_example_backfill.sql` >= 1
    - `grep -c "SET is_example = true" supabase/migrations/089_seed_is_example_backfill.sql` >= 1
    - `grep -c "cccccccc-0001-4000-8000-00000000000" supabase/migrations/089_seed_is_example_backfill.sql` == 16 (8 UUIDs × 2 occurrences — once in UPDATE, once in DO probe)
    - Inverted: `! grep -E "ALTER TABLE|CREATE TABLE|DROP TABLE|ALTER COLUMN|ADD COLUMN" supabase/migrations/089_seed_is_example_backfill.sql` (data-only — no DDL)
    - Inverted: `! grep "ON CONFLICT" supabase/migrations/089_seed_is_example_backfill.sql` (UPDATE not UPSERT — rows already exist by definition)
    - `grep -c "DO \\$\\$" supabase/migrations/089_seed_is_example_backfill.sql` >= 1 (post-update probe)
    - `grep -c "RAISE NOTICE" supabase/migrations/089_seed_is_example_backfill.sql` >= 1
  </acceptance_criteria>
...
<task type="checkpoint:human-action" gate="blocking">
  <name>Task 2: [BLOCKING] supabase db push — apply migration 089 to remote</name>
  <files>supabase/migrations/089_seed_is_example_backfill.sql (read-only — applied to remote DB; no source edits in this task), .planning/phases/13-discovery-v2-polish/TODOS.md (append "DISCO-05 backfill" section)</files>
  <read_first>
    - supabase/migrations/089_seed_is_example_backfill.sql (the migration to push)
  ...
  <what-built>
    Migration `supabase/migrations/089_seed_is_example_backfill.sql` is authored and committed. The remote Supabase project (`khslejtfbuezsmvmtsdn`) does NOT have this migration applied yet — it must be pushed before any verification or e2e test can prove DISCO-05's success criterion ("after data backfill, all 8 seed UUIDs have is_example=true").
...
2. Verify the migration file exists: `ls supabase/migrations/089_seed_is_example_backfill.sql`
...
- Migration: 089_seed_is_example_backfill.sql
...
|-----------|----------|-----------|-------------|-----------------|
| T-13-05-01 | Tampering (mass-flag of non-seed strategies as is_example) | Migration 089 UPDATE statement | mitigate | The WHERE clause uses `id IN (<8 hard-coded UUIDs>)` — no wildcards, no joins, no subqueries. The literal UUID list is verifiable against `scripts/seed-demo-data.ts:STRATEGY_UUIDS` via grep. The post-update DO block emits a NOTICE with the row count for observability. |
...
| T-13-05-03 | Denial of Service (UPDATE locks production tables) | Migration 089 UPDATE | accept | The UPDATE touches at most 8 rows (PK lookup); standard PostgreSQL row-level locks are released within milliseconds. No table scan, no FK cascades. |
...
- `grep -c "is_example = true" supabase/migrations/089_seed_is_example_backfill.sql` >= 1
- `grep -c "cccccccc-0001-4000-8000-00000000000" supabase/migrations/089_seed_is_example_backfill.sql` == 16 (8 UUIDs × 2 occurrences)
- `grep -c "test.skip" e2e/discovery-hide-examples-default.spec.ts` >= 1

**Negative checks:**
- `! grep -E "ALTER TABLE|CREATE TABLE|DROP TABLE" supabase/migrations/089_seed_is_example_backfill.sql` (data-only)
...
- Migration 089 file authored (cite line count + UUID grep count)
