...
| 164-03-01 | 03 | **3** | SHARE-01 | Non-owner mint / stale pre-image | Mint-or-**reuse** byte-identical across sessions; non-owner 404s; ⛔ token derived from the THREE-argument pre-image and round-tripped through `verifyShareToken` | route (**RED-first** on the reuse pin) | `npx vitest run src/app/api/strategies/[id]/share/route.test.ts && npx tsc --noEmit` | ❌ W0 | ⬜ pending |
| 164-03-02 | 03 | **3** | SHARE-03 | Revoke non-atomicity | Atomic `generation+1`; double-revoke converges via 404, never an error | route + audit-law | `npx vitest run src/app/api/strategies/[id]/share/revoke/route.test.ts && npx vitest run src/__tests__/audit-coverage.test.ts && npx tsc --noEmit` | ❌ W0 | ⬜ pending |
| 164-04-01 | 04 | **3** | SHARE-01, SHARE-04 | Published-lane regression | Published Copy Link stays `/factsheet/<id>?share=1` **byte-identical** (D-09) | component + url-shape pin | `set -o pipefail; npx vitest run "src/app/factsheet/[id]/v2" --silent 2>&1 \| tail -20 && npx vitest run src/__tests__/phase-148-owner-lane-cache-isolation.test.ts && npx tsc --noEmit` | ✅ | ⬜ pending |
| 164-04-02 | 04 | **3** | SHARE-04 | Affordance dishonesty | ONE predicate at all three sites; no "Link copied!" for a link that cannot work | component | `set -o pipefail; npx vitest run src/components/strategy src/app/\(dashboard\)/strategies --silent 2>&1 \| tail -20 && npx tsc --noEmit` | ✅ | ⬜ pending |
| 164-04-03 | 04 | **3** | SHARE-04 | Recipient chrome leak | Recipient never sees a Copy-Link control that rebuilds the URL without the token | component | `set -o pipefail; npx vitest run "src/app/factsheet/[id]/v2" --silent 2>&1 \| tail -20` | ✅ | ⬜ pending |
...
| 164-06-03 | 06 | 2 | SHARE-03 | Prod auto-apply | Harness run → three reviewers (execution status declared) → TEST hand-apply → `sql-tests` green | **blocking-human checkpoint** | none — `gate="blocking-human"`, typed `<resume-signal>` required | n/a | ✅ **CLEARED 2026-08-28** — 3 reviewers (all EXECUTED), 7 findings fixed + re-verified, TEST hand-apply done and verified; PROD confirmed byte-identical before/after |
