...
human_verification:
  ...
  - test: "Browser UAT of the full loop on a real server: mint, reuse, recipient render, revoke, dead link, published lane unchanged."
    ...
    result: "SUBSTANTIALLY DISCHARGED 2026-08-28 — the whole loop driven against PRODUCTION (quantalyze.xyz), authenticated as the owner, with every recipient read issued ANONYMOUSLY (credentials omitted) so the signed-out path is the one measured. Target was my own disposable test strategy 6f1a891f (QA159 409-remedy Delta, archived); nothing of the founder's was touched, and no share was left live. (1) AFFORDANCE, rendered: the strategies list shows `Get private link` on all three unpublished rows and `Share Factsheet` on both published rows — SHARE-04's isPublishedStatus split is correct in the real DOM, not just in jsdom. (2) MINT: 200, url with a 43-char base64url token. (3) REUSE: a second mint returned the IDENTICAL url — clicking Copy Link twice does not invalidate a link already sent, exactly what the route header claims (ON CONFLICT DO UPDATE SET revoked_at=NULL touches neither nonce nor generation). (4) RECIPIENT RENDER, ANONYMOUS: 200, 716,865 bytes, strategy name present. A signed-out reader sees the factsheet. (5) REVOKE: POST /share/revoke -> 200 {revoked:true}. (6) DEAD LINK: the same token now answers 410 GONE at /factsheet-share/gone, 425 bytes, name absent — a 410, not a 404, not a login redirect, and no content leak. (7) UNRESURRECTABLE: re-minting after revoke produced a DIFFERENT token, so the generation bumped and the old link stays dead — the property the migration exists to guarantee, confirmed live rather than argued. (8) NO ORACLE: a bogus 43-char token takes the identical 410 path, so a recipient cannot distinguish `revoked` from `never existed`. (9) PUBLISHED LANE UNCHANGED: /factsheet/<published-id>/v2 still renders 200 anonymously with its name, and the archived test strategy appears nowhere in it. ⛔ NOT VERIFIED, and honestly out of reach here: the clipboard WRITE itself and Safari transient-user-activation, plus any visual/layout judgement. The Chrome window in this environment reports viewport 0x0 and screenshots fail at the extension binding, so every element measures zero size and no pixel was ever rendered — the DOM was readable, the page was not viewable. Those three remain a genuine human item. ⚠️ FINDING RAISED BY THIS RUN, user-facing, filed to TODOS as [SHARE-HOST-01]: the minted url points at `https://quantalyze-rho.vercel.app`, NOT the branded `quantalyze.xyz` the owner is standing on. Cause is resolveAppUrl (route.ts:107-109), which prefers NEXT_PUBLIC_APP_URL over the request origin, and that variable is `https://quantalyze-rho.vercel.app` in Vercel Production AND Preview (measured from a clean directory). So every private link a user copies from the branded domain hands the recipient an unbranded host, and a Preview deploy mints Production-host links."
---
