...
human_verification:
  ...
  - test: "Set SHARE_TOKEN_SECRET (>= 32 chars) in Vercel for ALL environments, BEFORE merging this branch."
    ...
    result: "PASSED 2026-08-28 — SHARE_TOKEN_SECRET is present in ALL THREE Vercel environments (Production, Preview, Development) and is 64 characters, comfortably over the >=32 requirement. Verified without exposing the value: `vercel env run -e <env> -- sh -c` hashing in-subprocess, printing only length and a SHA-256 prefix. ⛔ FOLLOW-UP, NOT PART OF THIS ITEM: all three environments hold the IDENTICAL secret (fp=7fd1946fdd in each). That is precisely the arrangement WR-01 flags as wrong in .env.example — so the misguidance is not merely documented, it is REFLECTED IN LIVE CONFIG. A leaked Preview secret would mint share tokens that verify against Production. Rotating Preview and Development to distinct values is a founder action (I do not set secrets)."
