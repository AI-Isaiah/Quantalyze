...
status: executing
stopped_at: "164.8.3-04 complete — PHASE 164.8.3 PROBERAUTH IS DONE, 4 of 4 plans. ROADMAP criterion 6 is discharged on BOTH halves. (a) DURABLE, re-runs every CI shard: three `164.8.3 FALSIFIER` tests in `prod-prober-wiring.test.ts` build an in-memory mutant of `arms/mt5.mjs` with the `-6` branch excised (630 bytes, anchored on `  if (code === -6) {` through its block terminator, NEVER `indexOf`), and `6.txt` classifies as `mt5-terminal-error` through the mutant while the SHIPPING module returns `mt5-not-authorized` on subject `-6`. A surgical-excision control proves the SAME mutant still reads -10005 as `mt5-ipc-timeout` and -10004 as `mt5-no-ipc`, so the falsifier cannot go green on a mutant that classifies nothing. Split into THREE `it()`s on purpose — one RED is never credited to three controls. Nothing lands in the working tree: `mkdtempSync` + a `finally` `rmSync`, and `git status --porcelain -- scripts/` is asserted silent. (b) RECORDED, human-run: TWO on-disk levers observed RED through the real `--self-test`, each dropped ALONE with the other left live — lever 1 (the `-6` branch deleted, REMEDIES untouched) printed `SELF-TEST FAIL: the defect kind is mt5-not-authorized (got mt5-terminal-error)`; lever 2 (the branch left LIVE, count verified 1; `REMEDIES[\"mt5-not-authorized\"]` pointed at the EXACT `mt5-terminal-error` string) printed `SELF-TEST FAIL: no two mt5 remedies are the same string — two kinds with one remedy is two kinds pretending to be one`. Both exit 1; both restored from a `cp` byte backup RE-TAKEN between levers and `cmp`-verified (RESTORE-CMP-OK twice); `--self-test` re-run GREEN at 80/80 before the next lever. ⛔ `git checkout --` was never used. Each lever also reddened scenario 75/80, recorded as CORROBORATION of the same cause and never counted as a second control. The count HELD at 80 — no `scenario()` added. `[164.8.3-CAPTURE-MANIFEST-ERREXIT]` booked in TODOS.md as a NOMINATION to Phase 164.8.4 GATERESIDUE with that phase's `NOT A PRECEDENT` fence recorded; ROADMAP was NOT hand-edited. ⚠️ OUTSTANDING: the full `npm test` phase gate was not run by the executor (dispatch constraint), and `#753` has NO checkbox row in REQUIREMENTS.md (measured `grep -c` -> 0) so none was invented. PRIOR: 164.8.3-03 complete"
last_updated: "2026-09-13T17:58:00Z"
...
state_head: 7fbe70c9efb660825424187bb22dfec24651ed30
